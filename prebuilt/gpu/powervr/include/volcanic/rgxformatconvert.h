...
typedef enum _F16_ROUND_MODE
{
	F16_ROUND_MODE_RTZ,
	F16_ROUND_MODE_RNE,
	F16_ROUND_MODE_RNE_OVERFLOWTOMAX,
} F16_ROUND_MODE, *PF16_ROUND_MODE;

FORCE_INLINE 
IMG_UINT16 ConvertF32ToF16(IMG_FLOAT fV, F16_ROUND_MODE eRoundMode)
/*****************************************************************************
...
*****************************************************************************/
{
	...
	else if ((ui32Exponent > 0) && (ui32Exponent < 0xFF))  /* 0xFF = 255 */
	{
		...
		else if ((i32ExpVal >= -25) && (i32ExpVal <= -15)) /* 16-bit denormalised value needs to be converted from 32-bit normalised value */
		{
			IMG_UINT32 ui32FullMantissa = ui32Mantissa | (1 << 23);
			IMG_UINT32 ui32ExpShift = i32ExpVal + 24;
			IMG_UINT32 ui32MantissaShift = 23 - ui32ExpShift;
			IMG_UINT32 ui32LowMantissa  = ui32FullMantissa & ((1 << ui32MantissaShift) - 1);
			IMG_UINT32 ui32HalfWay = 1 << (ui32MantissaShift - 1);

			// 16-bit mantissa also depicts 32-bit mantissa value
			ui16Man = (IMG_UINT16)(ui32FullMantissa >> ui32MantissaShift);

			ui16Exp = 0;

			if (eRoundMode == F16_ROUND_MODE_RNE ||
				eRoundMode == F16_ROUND_MODE_RNE_OVERFLOWTOMAX)
			{
				/*
					Is the error >= 0.5ULP?
				*/
				if (ui32LowMantissa >= ui32HalfWay)
				{
					/*
						Round up if the error > 0.5ULP or the mantissa is odd.
					*/
					if (ui32LowMantissa > ui32HalfWay || (ui16Man & 1) != 0)
					{
						bPlusOne = IMG_TRUE;
					}
				}
		...
		else if ((i32ExpVal > 15)) /* overflow value set to the representative max */
		{
			if (eRoundMode == F16_ROUND_MODE_RTZ ||
				eRoundMode == F16_ROUND_MODE_RNE_OVERFLOWTOMAX)
			{
				ui16Exp = 0x1E;
				ui16Man = 0x03FF;
		...
		else /* 16-bit normalised value needs to be converted from 32-bit normalised value */
		{
			...
			if (eRoundMode == F16_ROUND_MODE_RNE ||
				(eRoundMode == F16_ROUND_MODE_RNE_OVERFLOWTOMAX && !(ui16Exp == 0x1e && ui16Man == 0x3ff)))
			{
				/*
					Is the error >= 0.5ULP?
				*/
				if (ui32Mantissa & 0x1000)
				{
					/*
						Round up if the error > 0.5ULP or the mantissa is odd.
					*/
					if (((ui32Mantissa & 0x1fff) > 0x1000) || (ui16Man & 1))
					{
						bPlusOne = IMG_TRUE;
					}
				}
