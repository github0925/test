...
typedef struct TQMPDumpSurfaceMemInfo_TAG
{
	PVRSRV_MEMDESC hMemDesc;
} TQMPDumpSurfaceMemInfo;

typedef struct TQMDumpSurfacesMemInfo_TAG
{
	TQMPDumpSurfaceMemInfo sSource;
	TQMPDumpSurfaceMemInfo sDestination;
} TQMDumpSurfacesMemInfo;

typedef struct TQMArgs_TAG
{
	PVRSRV_MEMINFO* psDstBufferToFree;
	PVRSRV_MEMINFO* psBufferToFree;
	...
	struct GMemoryUsageTAG *psDstBufferToFreeMemoryUsage;
	...
	TQMDumpSurfacesMemInfo** apsPDumpSurfacesMemInfo;
...
IMG_BOOL TQMQueueTransfer(RMContext* psCtx,
				 TQ_TDM_SOURCE *psSources,
                 TQ_TDM_SURFACE *psDestSurface,
