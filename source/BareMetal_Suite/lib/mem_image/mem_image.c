...
uint32_t mem_image_init(addr_t base, size_t sz)
{
    ...
    header->inited = 1;
    header->total_sz = sz;
    arch_clean_invalidate_cache_range((void *)base, info_sz);
    return 0;
}

static uint64_t calc_sz(struct __mem_image_entry_info *info,
                        uint32_t count)
{
    uint64_t total_sz = 0;
...
addr_t mem_image_get_entry(addr_t h, uint64_t addr_align,
                           uint64_t sz_align, uint64_t size,
                           const char *name, mem_image_entry_t *out_info)
{
    ...
    struct __mem_image_entry_info *info = (struct __mem_image_entry_info *)(
            h + sizeof(mem_image_header_t));
    arch_invalidate_cache_range((void *)h, HEADER_ENTRY_INFO_SIZE);
    ...
    addr_base = sizeof(mem_image_header_t) + sizeof(struct
                __mem_image_entry_info) * MAX_IMG_SEEKER_ENTRY_CNT;
    ...
    size = ROUNDUP(size, sz_align),
    addr += addr_base;
    addr = ROUNDUP(addr, addr_align);
    ...
    out_info->sz = size;
    arch_clean_invalidate_cache_range((void *)h, HEADER_ENTRY_INFO_SIZE);
out:
    return addr;
}
...
uint32_t mem_image_delete_entry(addr_t h, addr_t base)
{
    ...
    for (uint32_t i = 0; i < header->img_cnt; i++, info++) {
        if (info->addr_info.base == base
                && info->addr_info.base + info->addr_info.sz <= (h + header->total_sz)) {
...
uint32_t mem_image_seek(addr_t h, const char *name,
                        mem_image_entry_t *info)
{
    uint32_t name_len;
    mem_image_header_t *header = (mem_image_header_t *)h;
    struct __mem_image_entry_info *__info = (struct __mem_image_entry_info *)(
            h + sizeof(mem_image_header_t));
    arch_invalidate_cache_range((void *)h, HEADER_ENTRY_INFO_SIZE);
    name_len = strlen(name);
    ...
    if (header->img_cnt > MAX_IMG_SEEKER_ENTRY_CNT || name_len == 0
            || name_len > MAX_GPT_NAME_SIZE) {
        goto out;
    }
