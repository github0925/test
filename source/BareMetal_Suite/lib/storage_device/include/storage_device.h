...
struct storage_device {
    ...
    int (*read) (storage_device_t *storage_dev, uint64_t src, uint8_t *dst, uint64_t size);
    uint8_t * (*read_ptr) (storage_device_t *storage_dev, uint64_t src, uint64_t size);
    int (*write) (storage_device_t *storage_dev, uint64_t dst, const uint8_t *buf, uint64_t data_len);
...
storage_device_t *setup_storage_dev(enum storage_type type, uint32_t res_idx, void *config);
