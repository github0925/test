...
void gic_dump_reg(uint32_t GICD_BASE,uint32_t GICC_BASE)
{
    int addr=0;
    for(addr=0;addr<=0xD3c;addr+=4)
    {
        DBG("reg%08x=%08x\n",addr,mmio_read_32(GICD_BASE+addr));
    }
    for(addr=0;addr<=0xC;addr+=4)
    {
        DBG("reg%08x=%08x\n",addr,mmio_read_32(GICC_BASE+addr));
    }
}