...
void *i2c_handle;
const uint32_t dsi85_i2c_slave_addr = 0x2D;
...
static int panel_post_begin(void)
{
    ...
    /*2.After power is applied and stable, the DSI CLK lanes MUST
      ...
      to LP11 state */
    /*3.Set EN pin to Low*/
    pd->ops.output_enable(TCA9539_P01);
    pd->ops.output_val(TCA9539_P01, 0);
    mdelay(10);

    /*Wait 10ms*/
    /*4.Tie EN pin to high*/
    /*Wait 10ms*/
    pd->ops.output_val(TCA9539_P01, 1);
    mdelay(10);
    ...
    sn65dsi85_regs_init(i2c_handle, dsi85_i2c_slave_addr);

    /*6.Set the PLL_EN bit(CSR 0x0D.0)*/
    /*Wait 10ms*/
    /*7.Set the SOFT_RESET bit(CSR 0x09.0)*/
    /*Wait 10ms*/
    sn65dsi85_enable(i2c_handle, dsi85_i2c_slave_addr);
...
static int panel_post_end(void)
{
    ...
    uint32_t addr = dsi85_i2c_slave_addr;
...
struct sdm_panel mipi_hsd123_1920x720_lcd = {
    .panel_name = "mipi_hsd123_1920x720_lcd",
    .if_type = IF_TYPE_DSI,
