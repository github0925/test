...
struct Lontium_IC_Mode{
    uint8_t mipi_port_cnt; //1 or 2
    uint8_t mipi_lane_cnt; //1 or 2 or 4
    bool mipi_mode;   //dsi or csi
    uint8_t video_mode;    //Non-Burst Mode with Sync Pulses; Non-Burst Mode with Sync Events
    bool audio_out;   //i2s or spdif
    bool hdmi_coupling_mode;//ac_mode or dc_mode
    bool hdcp_encryption; //hdcp_enable or hdcp_diable
    bool hdmi_mode;
    uint8_t input_color_space;  //RGB888 or YUV422
};
...
struct cec_msg{
uint8_t cec_status;
uint8_t rx_data_buff[16];
uint8_t tx_data_buff[18];
uint8_t logical_address;
u16 physical_address;
uint8_t destintion;
uint8_t retries_times;
...
extern int LT9611_Init(void);
extern int LT9611_Chip_ID(void);
...
extern bool HDMI_WriteI2C_Byte(uint8_t RegAddr, uint8_t d);
#endif