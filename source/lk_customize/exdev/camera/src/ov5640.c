...
static int i2c_transmit(int bus, uint8_t addr, void *buf, size_t cnt,
                        bool start, bool stop)
{
    dprintf(OV5640_LOG,
            "i2c transmit: bus %d, addr 0x%02X, buf 0x%08X, cnt %lu\n",
            bus, addr, *(int *)buf, cnt);
    return 0;
}
status_t i2c_receive(int bus, uint8_t addr, void *buf, size_t cnt)
{
    dprintf(OV5640_LOG, "%s()\n", __func__);
    return 0;
}
#endif
...
static const struct ov5640_pixfmt ov5640_formats[] = {
    ...
    { V4L2_PIX_FMT_UYVY, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_YUYV, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_UYVYSP, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_YUYVSP, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_YUV444, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_YUV420SP, V4L2_COLORSPACE_SRGB, },
    { V4L2_PIX_FMT_YUV420XP, V4L2_COLORSPACE_SRGB, },
...
static inline struct ov5640_dev *to_ov5640_dev(struct v4l2_device *vdev)
{
...
static const struct reg_value ov5640_setting_15fps_XGA_1024_768[] = {
    {0x3035, 0x42, 0, 0}, {0x3036, 0x38, 0, 0}, {0x3c07, 0x08, 0, 0},
    {0x3c09, 0x1c, 0, 0}, {0x3c0a, 0x9c, 0, 0}, {0x3c0b, 0x40, 0, 0},
    {0x3814, 0x31, 0, 0},
    {0x3815, 0x31, 0, 0}, {0x3800, 0x00, 0, 0}, {0x3801, 0x00, 0, 0},
...
static int ov5640_init_slave_id(struct ov5640_dev *sensor)
{
    ...
    ret = hal_i2c_write_reg_data(i2c_handle, OV5640_DEFAULT_SLAVE_ID, buf, 2,
                                 buf + 2, 1);

    if (!ret) {
        dprintf(0, "%s: failed with %d\n", __func__, ret);
        return ret;
    }
...
static int ov5640_write_reg(struct ov5640_dev *sensor, u16 reg, u8 val)
{
    ...
    ret = hal_i2c_write_reg_data(i2c_handle, OV5640_DEFAULT_SLAVE_ID, buf, 2,
                                 buf + 2, 1);
    dprintf(OV5640_LOG, "%s(), ret=%d, buf[0]=0x%x-%x\n", __func__, ret,
            buf[0], buf[1]);
...
static int ov5640_read_reg(struct ov5640_dev *sensor, u16 reg16, u8 *val)
{
    ...
    ret = hal_i2c_read_reg_data(i2c_handle, OV5640_DEFAULT_SLAVE_ID, reg, 2,
                                buf, 1);
    ...
    dprintf(OV5640_LOG, "%s(): trans reg[0]=0x%x-%x, rece buf[0]=0x%x\n ",
            __func__, reg[0], reg[1],  buf[0]);
    *val = buf[0];
    return 0;
}
                 ...
                 unsigned int width, unsigned int height, bool nearest)
{
    ...
    mode = v4l2_find_nearest_size(struct ov5640_mode_info,
                                  ov5640_mode_data[fr],
...
static int ov5640_s_power(struct v4l2_device *vdev, int on)
{
    int ret = 0;
...
static int ov5640_get_fmt(struct v4l2_device *vdev,
                          struct v4l2_mbus_framefmt *fmt)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
...
static int ov5640_set_fmt(struct v4l2_device *vdev,
                          struct v4l2_mbus_framefmt format)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
    const struct ov5640_mode_info *new_mode;
    ...
    ret = ov5640_try_fmt_internal(sensor, mbus_fmt,
...
static int ov5640_set_ctrl_white_balance(struct ov5640_dev *sensor,
        int awb)
{
    int ret;
    ...
    if (!awb) {
        u16 red = (u16)sensor->ctrls.red_balance;
...
static int ov5640_set_ctrl_test_pattern(struct ov5640_dev *sensor,
                                        int value)
{
...
static int ov5640_g_volatile_ctrl(struct v4l2_device *vdev,
...
static int ov5640_s_ctrl(struct v4l2_device *vdev,
                         struct v4l2_ctrl *ctrl)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
    int ret;
...
static int ov5640_enum_frame_size(struct v4l2_frame_size_enum *fse)
{
    fse->min_width =
...
static int ov5640_g_frame_interval(struct v4l2_device *vdev,
                                   struct v4l2_fract *fi)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
    mutex_acquire(&sensor->lock);
...
static int ov5640_s_frame_interval(struct v4l2_device *vdev,
                                   struct v4l2_fract frame_interval)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
    const struct ov5640_mode_info *mode;
    ...
    frame_rate = ov5640_try_frame_interval(sensor, &frame_interval,
                                           mode->hact, mode->vact);
...
static int ov5640_s_stream(struct v4l2_device *vdev, int enable)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
    int ret = 0;
...
static int ov5640_set_interface(struct v4l2_device *vdev,
...
static int ov5640_get_interface(struct v4l2_device *vdev,
                                struct v4l2_fwnode_endpoint *ep)
{
    struct ov5640_dev *sensor = to_ov5640_dev(vdev);
...
static const struct v4l2_dev_ops ov5640_vdev_ops = {
...
static int ov5640_check_chip_id(struct ov5640_dev *sensor)
{
    ...
    dprintf(OV5640_LOG, "%s(): ret=%d, chip_id=0x%x.\n", __func__, ret,
            chip_id);

    if (ret) {
        dprintf(0, "%s: failed to read chip identifier\n",
                __func__);
        goto power_off;
    }

    if (chip_id != 0x5640) {
        dprintf(0, "%s: wrong chip identifier, expected 0x5640, got 0x%x\n",
                __func__, chip_id);
#if WITH_CSI_DUMMY_I2C
        ret = 0;
#else
...
struct v4l2_device *ov5640_init(int i2c_bus)
{
    ...
    sensor->i2c_client.adapter = i2c_bus + 2;
    sensor->i2c_client.addr = OV5640_DEFAULT_SLAVE_ID;
#if EMULATION_PLATFORM_FPGA
    hal_i2c_creat_handle(&i2c_handle, RES_I2C_I2C2);
#else

    if (i2c_bus == 2 || i2c_bus == 3)
        hal_i2c_creat_handle(&i2c_handle, RES_I2C_I2C9);
    else
        printf("ov5640 i2c bus error!\n");
    ...
    sensor->vdev.frame_interval.denominator =
    ...
    dprintf(OV5640_LOG, "ov5640_init() end\n");
    return &sensor->vdev;
...
/*
int camera_start(struct ov5640_dev *sensor)
{
    struct v4l2_frame_interval fi;

    ov5640_g_frame_interval(sensor, &fi);
    ov5640_s_frame_interval(sensor, &fi);

    ov560_set_fmt(sensor, sensor->vdev.fmt);
    ov5640_s_stream(sensor, 1);

    return 0;
}

int camera_stop(struct ov5640_dev *sensor)
{
    return ov5640_s_stream(sensor, 0);
}
*/
