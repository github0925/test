...
static lv_res_t decoder_info(struct _lv_img_decoder * decoder, const void * src, lv_img_header_t * header)
{
    ...
    //LV_LOG_WARN("decoder_info start \n");
     /*If it's a PNG file...*/
     if(src_type == LV_IMG_SRC_FILE) {
         ...
         if(!strcmp(&fn[strlen(fn) - 3], "png")) {              /*Check the extension*/
             ...
             //LV_LOG_WARN("decoder_info  res:%d \n",res);
             if(res != LV_FS_RES_OK) return -1;
             lv_fs_seek(&f, 16);
             uint32_t rn;
             lv_fs_read(&f, &size, 8, &rn);
             //LV_LOG_WARN("decoder_info rn:%d \n",rn);
...
static lv_res_t decoder_open(lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc)
{
    ...
    if(dsc->src_type == LV_IMG_SRC_FILE) {
        ...
        if(!strcmp(&fn[strlen(fn) - 3], "png")) {              /*Check the extension*/
            ...
            uint32_t* tmp = (uint32_t*)img_data;
        ...
        {
            LV_LOG_WARN("decoder_open check extension error :%s \n", dsc->src);
        }
