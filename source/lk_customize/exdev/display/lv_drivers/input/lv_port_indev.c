...
static bool touchpad_is_pressed(int screen_id);
static void touchpad_get_xy(lv_coord_t * x, lv_coord_t * y, int screen_id);
...
{
#ifdef SUPPORT_INPUT_SVC
    return indev_local_is_pressed(screen_id);
#else
    return false;
#endif
}
...
#ifdef SUPPORT_INPUT_SVC
	int x_, y_;
    indev_local_get_xy(&x_, &y_, screen_id);
