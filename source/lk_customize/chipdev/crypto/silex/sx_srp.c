...
static uint32_t genHash(uint32_t vce_id,
                        ...
                        hash_alg_t hash_alg)
{
    uint32_t status = hash_blk(hash_alg, vce_id, block_t_convert(NULL, 0, EXT_MEM), in, out);
                         ...
                         hash_alg_t hash_alg)
{
    ...
    status = hash_blk(hash_alg, vce_id, block_t_convert(NULL, 0, EXT_MEM), block_t_convert(hash_buf, (in1.len + in2.len), EXT_MEM), out);
    free(hash_buf);
                                  ...
                                  uint32_t cryptomem_idx)
{
    ...
    status = hash_blk(hash_alg, vce_id, block_t_convert(NULL, 0, EXT_MEM), usrpwd, x);
                          ...
                          uint32_t size)
{
    ...
    status = rng_get_rand_less_ref(vce_id, tmp, N);

    if (status) {
        return status;
    }

#else //trng not work, just workaround use IP pattern parameters
