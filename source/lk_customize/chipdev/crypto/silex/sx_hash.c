...
static void hash_run(uint32_t vce_id, hash_reg_config_t reg_config)
{
    uint32_t value;
    ...
    while (!(readl(_ioaddr(REG_INTSTAT_CE_(vce_id))) & 0x8)) {
        i++;

        if (i > 10) {
            LTRACEF("times: %d, CE_(%d) is busy, alg: 0x%x, state reg: 0x%x\n", i, vce_id, reg_config.mode, readl(_ioaddr(REG_INTSTAT_CE_(vce_id))));
        }
    }
    ...
    if (0 != value_state) {
        LTRACEF("CE_(%d) state: %d, error: 0x%x, control value: 0x%x\n", vce_id, (readl(_ioaddr(REG_STAT_CE_(vce_id))) & 0x8), value_state, value);
        //clear err status
...
static uint32_t sx_hash_internal(hash_alg_t hash_alg,
                                 uint32_t ce_id,
                                 block_t extra_in,
                                 block_t key,
                                 ...
                                 block_t data_out)
{
    ...
    LTRACEF("data_in.addr= %p\n", data_in.addr);

    if (EXT_MEM == reg_config.dst_type) {
        reg_config.src_addr = _vaddr(addr_switch_to_ce(ce_id, data_in.addr_type, (_paddr((void*)data_in.addr))));
    }
    else {
    ...
    if (EXT_MEM == reg_config.dst_type) {
        reg_config.dst_addr = _vaddr(addr_switch_to_ce(ce_id, data_out.addr_type, (_paddr((void*)data_out.addr))));
    }
    else {
    ...
    if ((EXT_MEM == data_out.addr_type) && (NULL != data_out.addr)) {
        ...
        ret = memcpy_blk_cache(ce_id, block_t_convert((void*)data_out.addr, hash_len, data_out.addr_type),
                               block_t_convert((void*)CE2_SRAM_PUB_HASH_OUT_ADDR_OFFSET, hash_len, SRAM_PUB), hash_len, false, true);
    ...
    block_t data_in)
{
    ...
    if (0 != (data_in.len & (blocksize - 1))) {
        LTRACEF("hash_update_blk check data_in not align.\n");
        return CRYPTOLIB_INVALID_PARAM;
    }

    if (first_part) {
        iv = block_t_convert(NULL, 0, SRAM_PUB);
    }
    else {
    ...
    uint32_t total_len)
{
    ...
    iv = block_t_convert((uint8_t*)CE2_SRAM_PUB_HASH_MULT_PART_UPDATE_IV_ADDR_OFFSET, digest_size, SRAM_PUB);
