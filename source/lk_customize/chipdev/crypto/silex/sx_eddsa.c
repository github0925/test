...
static uint32_t eddsa_load_parameters(uint32_t vce_id, eddsa_curve_t curve)
{
    pke_load_curve(vce_id,
                   block_t_convert(ecc_eddsa_params[curve], ecc_eddsa_param_size[curve], EXT_MEM),
                                     ...
                                     const size_t result_size)
{
    ...
    if (curve == SX_ED25519) {
        status = hash_blk(ALG_SHA512, vce_id, block_t_convert(NULL, 0, 0), sk, block_t_convert(result, result_size, EXT_MEM));
        ...
        status = hash_blk(ALG_SHA256, vce_id, block_t_convert(NULL, 0, 0), sk, block_t_convert(result, ED448_SIGN_SIZE, EXT_MEM));
    ...
    block_t pk)
{
    ...
    point2CryptoRAM_rev(vce_id, block_t_convert(buffer, key_size * 2, EXT_MEM),
    ...
    CryptoRAM2point_rev(vce_id, block_t_convert(buffer, key_size * 2, EXT_MEM),
