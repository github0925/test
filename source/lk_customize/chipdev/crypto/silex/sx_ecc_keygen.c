...
uint32_t ecc_validate_public_key(uint32_t vce_id,
                                 block_t domain,
                                 block_t pub,
                                 uint32_t size,
                                 uint32_t curve_flags)
{
    ...
    pke_set_command(vce_id, BA414EP_OPTYPE_ECC_CHECK_POINTONCURVE, size, BA414EP_LITTLEEND, curve_flags);
    pke_load_curve(vce_id, domain, size, BA414EP_LITTLEEND, 1);
    point2CryptoRAM_rev(vce_id, pub, size, BA414EP_MEMLOC_6);

    pke_set_config(vce_id, BA414EP_MEMLOC_6, BA414EP_MEMLOC_6, BA414EP_MEMLOC_6, 0x0);
                              ...
                              uint32_t curve_flags)
{
    ...
    //block_t n = block_t_convert(domain.addr + size, size, domain.addr_type);
    uint32_t status = ecc_generate_private_key(vce_id, n, priv);
                                 ...
                                 uint32_t curve_flags)
{
    ...
    pke_set_dst_param(vce_id, size, 0x1 << BA414EP_MEMLOC_6 | 0x1 << BA414EP_MEMLOC_7, (_paddr((void*)pub.addr)), pub.addr_type);
#endif
