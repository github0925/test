                      ...
                      bool* is_prime)
{
    ...
    for (uint32_t i = 0; i < iteration; i++) {
        ...
        uint32_t status = rng_get_rand_less_ref(vce_id, // 0 < rand < number-2
                                                block_t_convert(rand, length, EXT_MEM), candi_prime);
        ...
        status = miller_rabin_round(vce_id, block_t_convert(rand, length, EXT_MEM), candi_prime, is_prime);
 ...
 */
static uint32_t get_prime_modulo(uint32_t vce_id, uint8_t* number, uint32_t nr_of_bytes, uint32_t* mod)
{
    ...
    uint32_t status = modular_reduce_odd(vce_id,
                                         block_t_convert(number, nr_of_bytes, EXT_MEM),
                                         block_t_convert(tmp, nr_of_bytes, EXT_MEM),
                                         block_t_convert(tmp, nr_of_bytes, EXT_MEM));
 ...
 * @return CRYPTOLIB_SUCCESS if successful
 */
static uint32_t is_coprime(uint32_t vce_id, uint8_t* number, uint32_t nr_of_bytes, bool* coprime)
{
    uint8_t* tmp;// working buffer for intermediate steps
    struct mem_node* mem_n;
    ...
    if (nr_of_bytes < prodsize) {
        ...
        memcpy_blk(vce_id, block_t_convert(tmp + prodsize - nr_of_bytes, nr_of_bytes, EXT_MEM),
                   block_t_convert(number, nr_of_bytes, EXT_MEM), nr_of_bytes);
        number = tmp;
    }
     ...
     */
        ...
        memcpy_blk(vce_id, block_t_convert(tmp + nr_of_bytes - prodsize, prodsize, EXT_MEM),
                   block_t_convert(prod_first_130_odd_primes, prodsize, EXT_MEM), prodsize);
    ...
    status = modular_inverse_odd(vce_id, block_t_convert(number, prodsize, EXT_MEM),
                                 block_t_convert((uint8_t*)prod_first_130_odd_primes, prodsize, EXT_MEM),
                                 block_t_convert(tmp, prodsize, EXT_MEM));
                           ...
                           uint32_t rounds)
{
    ...
    *(candi_number.addr + candi_number.len - 1) |= 0x01;
     ...
     */
    uint32_t status = get_prime_modulo(vce_id, candi_number.addr, candi_number.len, &mod);
    ...
    while (1) {
        ...
        if (possibly_prime) {
             ...
             */
            bool coprime = false;
            status = is_coprime(vce_id, candi_number.addr, candi_number.len, &coprime);
