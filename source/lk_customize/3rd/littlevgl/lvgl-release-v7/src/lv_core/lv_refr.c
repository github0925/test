...
static uint32_t elaps_flush = 0;
static uint32_t elaps_calc = 0;
static uint32_t elaps_memcpy = 0;
static uint32_t elaps_while = 0;
/**
...
void _lv_disp_refr_task(lv_task_t * task)
{
    ...
    if(disp_refr->inv_p != 0) {
        ...
        if(lv_disp_is_true_double_buf(disp_refr)) {
            if(disp_refr->driver.set_px_cb) {
                ...
                elaps_while = lv_tick_get() - start - elaps_flush - elaps_calc;
                ...
                LV_UNUSED(hres);
                lv_gpu_sdrv_g2d_copy((lv_color_t *)(buf_act), hres, (lv_color_t *)buf_ina, hres, hres, lv_disp_get_ver_res(disp_refr));
                //memcpy((lv_color_t *)(buf_act), (lv_color_t *)(buf_ina), lv_disp_get_hor_res(disp_refr) * lv_disp_get_ver_res(disp_refr) * 4);
                // lv_gpu_sdrv_g2d_blend(&vdb->area, (lv_color_t *)(buf_act), &vdb->area, &vdb->area, (lv_color_t *)(buf_ina), LV_OPA_COVER, NULL, LV_DRAW_MASK_RES_FULL_COVER);
#else //memcpy invalidate display areas.
                ...
                if (index == 500 && disp_refr->inv_p > 0)
                {
                    //LV_LOG_WARN("disp_refr->inv_p:%d, joined:%d, join_num:%d, y1:%d, w:%d, h:%d", disp_refr->inv_p, disp_refr->inv_area_joined[1], join_num, disp_refr->inv_areas[1].y1, lv_area_get_width(&disp_refr->inv_areas[1]), lv_area_get_height(&disp_refr->inv_areas[1]));
                }
                if(copy_buf) _lv_mem_buf_release(copy_buf);
#endif
            }

            elaps_memcpy = lv_tick_get() - start - elaps_flush - elaps_calc - elaps_while;
        ...
        {
            index = 0;
            //LV_LOG_WARN("lv_tick_elaps %d, elaps_calc:%d, elaps_flush:%d, elaps_while:%d, elaps_memcpy:%d, loop_gap:%d",  elaps, elaps_calc, elaps_flush, elaps_while, elaps_memcpy, start-lasttime);
        }

        lasttime = lv_tick_get();
    ...
    if(perf_label == NULL) {
        ...
        lv_obj_set_style_local_bg_color(perf_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_ORANGE);
    ...
    if(top_act_scr == NULL && top_prev_scr == NULL) {
        if(disp_refr->bg_img) {
            ...
            if (dsc.bg_opa <= LV_OPA_MIN){
                lv_clean_rect_transp(&start_mask);
            }
            else
#endif
            {
...
static void lv_refr_vdb_flush(void)
{
    ...
    elaps_flush = lv_tick_get() - start;
}

void lv_set_flush_enabled(bool enabled)
{
    LV_LOG_WARN("lv_set_flush_enabled : %d", enabled);
