                                                       ...
                                                       const lv_draw_img_dsc_t * draw_dsc)
{
     ...
     * Just draw it!*/
        ...
        //LV_LOG_WARN("draw_core img data %d\n", lv_tick_get() - start);
    }
    /* The whole uncompressed image is not available. Try to read it line-by-line*/
        ...
        _lv_mem_buf_release(buf);
        //LV_LOG_WARN("draw_core not img %d\n", lv_tick_get() - start);
    }
                                              ...
                                              bool chroma_key, bool alpha_byte)
{
...
#if LV_USE_GPU_SEMIDRIVE_G2D & 0
			/*Blend ARGB images directly*/
			if(lv_area_get_height(&draw_area) > 20) {
				lv_gpu_sdrv_g2d_blend(disp_area, vdb->buf_act, clip_area, map_area, (const lv_color_t *)map_p,
					draw_dsc->opa, NULL, LV_DRAW_MASK_RES_CHANGED);
                //LV_LOG_WARN("lv_draw_map g2d blend %d\n", lv_tick_get() - start);
				return;
			}
#endif
    ...
    uint32_t gap = lv_tick_get() - start;
    if (gap > 3)
    {
        //LV_LOG_WARN("lv_draw_map cpu %d\n", gap);
    }
