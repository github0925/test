                                              ...
                                              const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        ...
        // cover BLEND_PIXEL_NONE
        lv_gpu_sdrv_dma2d_fill(disp_buf_first, disp_w, color, draw_area_w, draw_area_h, opa);
        return;
#endif
        if(opa > LV_OPA_MAX) {
#if LV_USE_GPU
...
#if LV_USE_GPU_SEMIDRIVE_G2D && 0
                if(lv_area_get_size(draw_area) > 240) {
                    lv_gpu_sdrv_dma2d_fill_mask(disp_buf_first, disp_w, color, mask, opa, draw_area_w, draw_area_h);
                    return;
                }
#endif
                                             ...
                                             const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
#if LV_USE_GPU_SEMIDRIVE_G2D & 0
        if(lv_area_get_height(draw_area) > 10) {
            lv_gpu_sdrv_g2d_blend(disp_area, disp_buf, draw_area, map_area, map_buf, opa, mask, mask_res);
            return;
        }
#endif
