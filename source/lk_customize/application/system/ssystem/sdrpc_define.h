#ifndef __SDRPC_DEFINE_H__
#define __SDRPC_DEFINE_H__

//generic register using at boot stage for sync status between cores
#define SEC_WRITE_REG (0xf8400000 + (0x6c << 10))
#define SAF_WRITE_REG (0xf8400000 + (0x68 << 10))
#define AP_WRITE_REG  (0xf8400000 + (0x64 << 10))

typedef enum sdrpc_op_type {
    SDRPC_NOTIFY = 0,
    SDRPC_READ = 1,
    SDRPC_RESPONSE = 2,
    SDRPC_OP_MAX
...
uint32_t post_message(sdrpc_commands_type command, sdrpc_op_type operation, void * payload, uint32_t payload_size, uint32_t object);
uint32_t receive_message(sdrpc_commands_type command, sdrpc_op_type operation, void * payload, uint32_t payload_size);
