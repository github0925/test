...
/* DEBUG_IPL is defined in project mk */
#if DEBUG_IPL == 1
...
struct startup_header {
    ...
    _Uint32t
    imagefs_paddr;      /*IS Set by IPL to where the imagefs is when startup runs*/
...
struct startup_trailer {
    _Uint32t
    cksum;  /* Checksum from start of header to start of trailer */
};
