...
int32_t get_bpt_info(struct bpt *bpt, uint8_t *buffer, uint32_t len)
{
    ...
    if (!bpt || !buffer) {
        dprintf(CRITICAL, "%s bpt/buffer is null\n", __func__);
        return 1;
    }

    if (len < BPT_SIZE) {
        dprintf(CRITICAL, "%s bpt size error! size:%d\n", __func__, len);
        return 1;
    }

    if (*(uint32_t *)temp != BPT_TAG) {
        dprintf(CRITICAL, "%s bpt tag error!\n", __func__);
        return 1;
    }
    ...
    if (bpt->size != BPT_SIZE) {
        dprintf(CRITICAL, "%s bpt size error!\n", __func__);
        return 1;
    }
    ...
    if (iib->tag != IIB_TAG) {
        dprintf(CRITICAL, "%s iib tag error!\n", __func__);
        return 1;
    }
    ...
    rcp = &bpt->rcp;
    rcp->tag = *(uint16_t *)(temp + RCP_TAG);

    if (rcp->tag != RCP_TAG) {
        dprintf(CRITICAL, "%s rcp tag error!\n", __func__);
        return 1;
    }
    ...
    if (rcp->size != RCP_SIZE) {
        dprintf(CRITICAL, "%s rcp size error:%d\n", __func__, rcp->size);
        return 1;
    }
