...
#if WITH_KERNEL_VM
#define FPGA_DDR_PFMON_BASE 0xFFFF000033020000
#endif
...
#define master_mask 0x7f0000
#define DDR_clock 1066000000
#define time_window_per_ms 1066000
...
struct mem_pool {
    void *address_start;
    bool need_record;
    uint32_t record_limite;    // pool_size/sizeof(record)
    uint32_t records;
...
struct channel_head {
    enum master_id master1;
    enum master_id master2;
    unsigned long offset;
    unsigned long pool_size;
};

//[PFM_HEAD][PFM_RECORD][EVENT_GURAD][PFM_EVENT]
#define PFM_EVENT_BUFFER_SIZE 0x1000
#define PFM_MAX_EVENT 256
#define PFM_GUARD_PATTERN 0x5a5a5a5a
#define PFM_EVENT_GUARD_SIZE 0

struct pfm_event_head {
    lk_time_t pfm_start_time;
...
struct pfm_event {
    int index;
    enum pfm_event_type type;
    enum master_id master;
    lk_time_t occurred;
};

struct pfm_head {
//global
    uint32_t global_parrten; //for head parse check
    enum pfm_mode mode;
    uint32_t active_group;
    uint32_t time_window;
    vaddr_t pool_address;
    unsigned long pool_size;
    unsigned long pfm_collect_size;
    //for IRQ
    uint32_t current_record;
    uint32_t records;

    //channel
    struct channel_head channel[16];
    struct pfm_event_head event_head;
    uint64_t pfm_head_gurad;

};

int pfm_notice_underrun(enum master_id);

int pfm_notice_overflow(enum master_id);

int pfm_notice_gpu_begin(enum master_id);

int pfm_notice_gpu_end(enum master_id);
