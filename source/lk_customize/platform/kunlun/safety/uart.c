...
#define SAFETY_UART_SCLK CKGEN_UART_SAF  /* 80M */
#define SECURE_UART_SCLK 60000000        /* 60M */
#define UART_BAUD   115200

static cbuf_t uart_rx_buf;
static void *safety_log_handle = NULL;
...
void uart_update_rx_cbk(hal_uart_int_callback cbk)
{
    if(cbk == NULL)
        cbk = save_char;
    hal_uart_int_src_disable(safety_log_handle,
                                 UART_HAL_RX_CHAR_INT_SRC);
    hal_uart_int_cbk_register(safety_log_handle, UART_HAL_RX_CHAR_INT_SRC,
                                  cbk);
...
{
    if (safety_log_handle) {
	hal_uart_irq_mask(safety_log_handle);
        hal_uart_release_handle(safety_log_handle);
        safety_log_handle = NULL;
    }
}
