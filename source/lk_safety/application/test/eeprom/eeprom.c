                 ...
                 const uint8_t *buf, uint64_t data_len)
{
    int ret = 0;
    struct eeprom_data *data = storage_dev->priv;
    ret = hal_i2c_write_reg_data(data->i2c_handle, data->slave_addr,
                ...
                uint8_t *dst, uint64_t data_len)
{
    int ret = 0;
    struct eeprom_data *data = storage_dev->priv;
    ret = hal_i2c_read_reg_data(data->i2c_handle, data->slave_addr,
                          (void *)&src, 1, (void *)dst, data_len);
    return ret;
}

struct eeprom_data eeprom_dev_data[2] = {
    {RES_I2C_I2C1, 0x50, NULL},
    {RES_I2C_I2C1, 0x51, NULL},
};
