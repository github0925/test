...
$(info "----------1------------- $(LOCAL_DIR)")
PROJECT :=$(SD_PROJECT)
include $(LOCAL_DIR)/../chipcfg/generate/$(CHIPVERSION)/projects/$(SD_PROJECT)/ospi_handover_cfg.mk


SUPPORT_RPC = false
SUPPORT_RPMSG_SAMPLE_CODE = false
SUPPORT_3RD_RPMSG_LITE = false
SUPPORT_IPCC_RPMSG = false
SUPPORT_DCF = false
SUPPORT_BOARDINFO := false
SUPPORT_FATFS := false
SUPPORT_CPU_SDDRV := true
SUPPORT_UNIFIED_BOOT := false
SUPPORT_PRINT_BUILDCONFIG ?= false

SUPPORT_MODULE_HELPER_SDDRV := false
MODULE_HELPER_PER_DISP := false
SUPPORT_DISP_SDDRV := false
STATIC_HANDOVER := true
SUPPORT_NEXT_OS := false

PRELOAD_RES_SIZE := 0x0
GLOBAL_DEFINES +=STATIC_HANDOVER=1
GLOBAL_DEFINES += NOT_USE_SYS_CFG=1
GLOBAL_DEFINES += SUPPORT_DIL2_INIT=1

##########################
#copy from common dil2
##########################
