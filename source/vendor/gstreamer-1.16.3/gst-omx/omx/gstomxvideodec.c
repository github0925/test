...
gst_omx_video_dec_allocate_output_buffers_ext (GstOMXVideoDec * self);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_USE_DMABUF:
      self->dmabuf = g_value_get_boolean (value);
      break;
    default:
...
#endif
    case PROP_USE_DMABUF:
      g_value_set_boolean (value, self->dmabuf);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_omx_video_dec_class_init (GstOMXVideoDecClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_USE_DMABUF,
      g_param_spec_boolean ("use-dmabuf", "DMABUF",
          "Whether to use dmabuf for buffer zero copy with sink ", FALSE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_omx_video_dec_allocate_output_buffers_ext (GstOMXVideoDec * self)
{
  ...
  if (!self->dmabuf)
    return gst_omx_port_allocate_buffers (port);
  ...
  if (pool) {
    GST_DEBUG_OBJECT (self, "get config from downstream pool");
     ...
     * TODO: check whether match then reconfigure
     */
    min = MAX (min, port_def.nBufferCountMin);
    if (max == 0) {
      max = min;
    } else if (max < min) {
    ...
    min = max = port_def.nBufferCountMin;
    GST_DEBUG_OBJECT (self, "No pool available, not negotiated yet");

    caps = gst_pad_get_pad_template_caps (GST_VIDEO_DECODER_SRC_PAD (self));
    add_videometa = TRUE;
  }

  if (caps) {
    GST_DEBUG_OBJECT (self, "downstream caps: %" GST_PTR_FORMAT, caps);

    GST_DEBUG_OBJECT (self, "create out_port_pool as %s",
        self->dmabuf ? "DMABUF" : "SYSTEMMEMORY");

    GstVideoInfo info;
    if (!gst_video_info_from_caps (&info, caps)) {
      GST_ERROR_OBJECT (self, "get video info failed");
      err = OMX_ErrorUndefined;
      goto done;
    }
    ...
    GST_ERROR_OBJECT (self, "outpool config %p", config);
    if (config) {
      gst_buffer_pool_config_set_params (config, caps,
          port_def.nBufferSize, min, 0);
      if (add_videometa)
        gst_buffer_pool_config_add_option (config,
            GST_BUFFER_POOL_OPTION_VIDEO_META);

      GST_ERROR_OBJECT (self, "gst_buffer_pool_set_config, min %d, max %d",
          min, max);
      if (!gst_buffer_pool_set_config (self->out_port_pool, config)) {
        GST_ERROR_OBJECT (self, "set pool config failed");
        err = OMX_ErrorUndefined;
        goto failed;
      }

      GList *buffers = GST_OMX_BUFFER_POOL (self->out_port_pool)->prime_fds;
      gint i;
      for (i = 0; i < port_def.nBufferCountMin; i++) {
        guint prime_fd;
        if (!gst_omx_buffer_bo_alloc (self->out_port_pool, &info, &prime_fd)) {
          err = OMX_ErrorUndefined;
          goto failed;
        }

        guint *pfd = g_new (guint, 1);
        *pfd = prime_fd;
        GST_DEBUG_OBJECT (self, "buffer[%d] prime_fd %u", i, prime_fd);

        buffers = g_list_prepend (buffers, pfd);
      }

      if (buffers) {
        OMX_PARAM_BUFFERSUPPLIERTYPE bufferSupplierType;
        GST_OMX_INIT_STRUCT (&bufferSupplierType);

        bufferSupplierType.nPortIndex = port->port_def.eDir;
        bufferSupplierType.eBufferSupplier = OMX_BufferSupplyVendorDRM;

        err = gst_omx_component_set_parameter (self->dec,
            OMX_IndexParamCompBufferSupplier, &bufferSupplierType);
        if (err != OMX_ErrorNone) {
          goto failed;
        }
    ...
    failed:
      {
        if (buffers) {
          GList *l = buffers;
          guint n = g_list_length ((GList *) buffers);
          for (i = 0; i < n; i++) {
            guint *prime_fd = (guint *) l->data;
            gst_omx_buffer_bo_free (self->out_port_pool, *prime_fd);
            l = l->next;
          }
        }
...
gst_omx_video_dec_negotiate (GstOMXVideoDec * self)
{
  ...
  if (self->dmabuf) {
    gst_caps_set_features (comp_supported_caps, 0,
  ...
  if (!gst_pad_has_current_caps (GST_VIDEO_DECODER_SRC_PAD (self))) {
    ...
    GST_DEBUG_OBJECT (self,
        "try to negotiate output state caps: %" GST_PTR_FORMAT, state->caps);
...
gst_omx_video_dec_enable (GstOMXVideoDec * self, GstBuffer * input)
{
  ...
  if (self->disabled) {
    ...
    if (!(klass->cdata.hacks & GST_OMX_HACK_NO_DISABLE_OUTPORT)) {
      ...
      if (!gst_omx_video_dec_allocate_in_buffers (self))
        return FALSE;
      if (gst_omx_video_dec_allocate_output_buffers_ext (self) != OMX_ErrorNone)
        return FALSE;
    }
