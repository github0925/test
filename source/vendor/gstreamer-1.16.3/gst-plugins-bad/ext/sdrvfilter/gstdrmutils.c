...
vendor_alloc_buffer (struct dm_drm_t *drv, struct dm_drm_handle_t *handle)
{
  ...
  if (ret) {
    GST_DEBUG_OBJECT (NULL, "Failed to create drm dumb buffer.");
    return -1;
  }
  ...
  if (ret) {
    GST_DEBUG_OBJECT (NULL, "Failed to ioctl to fd.");
    return -1;
  }
...
dm_drm_create (void)
{
  ...
  drm->fd = open (DRM_DEVICE, O_RDWR);
  if (drm->fd < 0) {
    GST_DEBUG_OBJECT (NULL, "failed to open %s", DRM_DEVICE);
    free (drm);
    return NULL;
  }
...
/* DEF_FMT (XBGR1555, ???), */
  ...
  DEF_FMT (XBGR8888, RGBx), DEF_FMT (BGR888, RGB),    DEF_FMT (RGB888, BGR),
#else
...
#endif
  DEF_FMT (UYVY, UYVY),     DEF_FMT (YUYV, YUY2),     DEF_FMT (YVYU, YVYU),
  DEF_FMT (YUV420, I420),   DEF_FMT (YVU420, YV12),   DEF_FMT (YUV422, Y42B),
  DEF_FMT (NV12, NV12),     DEF_FMT (NV21, NV21),     DEF_FMT (NV16, NV16),
