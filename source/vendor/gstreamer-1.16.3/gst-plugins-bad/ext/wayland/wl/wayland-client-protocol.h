/*
 * Copyright  2008-2011 Kristian Hogsberg
...
extern const struct wl_interface wl_vibrator_interface;
...
struct wl_callback_listener {
	 ...
	 * @fd: (none)
	 *
	 * Notify the client when the related request is done. With
	 * fence.
	 */
	void (*done_fence)(void *data,
			   struct wl_callback *wl_callback,
			   uint32_t callback_data,
			   int32_t fd);
};
...
#define WL_CALLBACK_REQUEST_DONE	0
...
static inline void
wl_callback_request_done(struct wl_callback *wl_callback)
{
	wl_proxy_marshal((struct wl_proxy *) wl_callback,
			 WL_CALLBACK_REQUEST_DONE);
}

/**
 * wl_compositor - the compositor singleton
 * @prepare_surface: notify clients it's ready to create surface
 * @producerfd: producer fd
 * @bq_socket_name: buffer queue socket name
 *
...
struct wl_compositor_listener {
	 ...
	 * 
	 */
	void (*prepare_surface)(void *data,
	 ...
	 * @producerfd: (none)
	 *
	 * Sent producer fd to client, client use this producer fd to
	 * create buffer queue producer.
	 */
	void (*producerfd)(void *data,
			   struct wl_compositor *wl_compositor,
			   int32_t producerfd);
	/**
	 * bq_socket_name - buffer queue socket name
	 * @socket_name: (none)
	 *
	 * Sent buffer queue socket name to client, client use this
	 * socket name to create buffer queue producer.
	 */
	void (*bq_socket_name)(void *data,
			       struct wl_compositor *wl_compositor,
			       const char *socket_name);
};

static inline int
wl_compositor_add_listener(struct wl_compositor *wl_compositor,
			   const struct wl_compositor_listener *listener, void *data)
{
	return wl_proxy_add_listener((struct wl_proxy *) wl_compositor,
				     (void (**)(void)) listener, data);
}

#define WL_COMPOSITOR_CREATE_SURFACE	0
#define WL_COMPOSITOR_CREATE_REGION	1
#define WL_COMPOSITOR_SET_POWER_MODE	2
#define WL_COMPOSITOR_CREATE_SURFACE_V2	3
#define WL_COMPOSITOR_CREATE_SURFACE_FD	4
...
static inline void
wl_compositor_set_power_mode(struct wl_compositor *wl_compositor, uint32_t value)
{
	wl_proxy_marshal((struct wl_proxy *) wl_compositor,
			 WL_COMPOSITOR_SET_POWER_MODE, value);
}

static inline struct wl_surface *
wl_compositor_create_surface_v2(struct wl_compositor *wl_compositor, uint32_t width, uint32_t height)
{
	struct wl_proxy *id;

	id = wl_proxy_marshal_constructor((struct wl_proxy *) wl_compositor,
			 WL_COMPOSITOR_CREATE_SURFACE_V2, &wl_surface_interface, NULL, width, height);

	return (struct wl_surface *) id;
}

static inline struct wl_surface *
wl_compositor_create_surface_fd(struct wl_compositor *wl_compositor, uint32_t width, uint32_t height, int32_t efd)
{
	struct wl_proxy *id;

	id = wl_proxy_marshal_constructor((struct wl_proxy *) wl_compositor,
			 WL_COMPOSITOR_CREATE_SURFACE_FD, &wl_surface_interface, NULL, width, height, efd);
 ...
 * @release_fence: compositor releases buffer
 *
...
struct wl_buffer_listener {
	 ...
	 * @fd: (none)
	 *
	 * Sent release_fence to client, client access this buffer need
	 * wait until this fence signaled.
	 */
	void (*release_fence)(void *data,
			      struct wl_buffer *wl_buffer,
			      int32_t fd);
};
...
#define WL_SHELL_WM_GET_SHELL_SURFACE	1
...
static inline struct wl_shell_surface *
wl_shell_wm_get_shell_surface(struct wl_shell *wl_shell, struct wl_surface *surface, int32_t surface_type, int32_t surface_pos, int32_t taskid, int32_t stackid)
{
	struct wl_proxy *id;

	id = wl_proxy_marshal_constructor((struct wl_proxy *) wl_shell,
			 WL_SHELL_WM_GET_SHELL_SURFACE, &wl_shell_surface_interface, NULL, surface, surface_type, surface_pos, taskid, stackid);
...
#ifndef WL_SHELL_SURFACE_ATTRIBUTE_MASK_ENUM
#define WL_SHELL_SURFACE_ATTRIBUTE_MASK_ENUM
/**
 * wl_shell_surface_attribute_mask - shell_surface attribute mask
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_HAS_STATUS_BAR: (none)
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_SHOW_STATUS_BAR: (none)
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_WESTON_RESIZE_WHEN_NEED_UPDATE_LAYOUT: (none)
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_SET_TYPE_GUEST_SURFACETEXTURE: set
 *	shell_surface type to
 *	WINDOW_MANAGER_SHELL_SURFACE_TYPE_GUEST_SURFACETEXTURE
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_SET_TYPE_VIDEO_SURFACETEXTURE: set
 *	shell_surface type to
 *	WINDOW_MANAGER_SHELL_SURFACE_TYPE_VIDEO_SURFACETEXTURE
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_GUEST_SURFACETEXTURE_ATTRIB_ABOVE: 
 *	set shell_surface flag to have TYPE_GUEST_SURFACETEXTURE window above it
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_VIDEO_SURFACETEXTURE_ATTRIB_ABOVE: 
 *	set shell_surface flag to have TYPE_VIDEO_SURFACETEXTURE window above it
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_GUEST_SURFACETEXTURE_ATTRIB_BELOW: 
 *	set shell_surface flag to have TYPE_GUEST_SURFACETEXTURE window blow it
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_VIDEO_SURFACETEXTURE_ATTRIB_BELOW: 
 *	set shell_surface flag to have TYPE_VIDEO_SURFACETEXTURE window blow it
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_CLEAR_GUEST_SURFACETEXTURE_ATTRIB: 
 *	clear shell_surface flag to have TYPE_GUEST_SURFACETEXTURE window
 * @WL_SHELL_SURFACE_ATTRIBUTE_MASK_CLEAR_VIDEO_SURFACETEXTURE_ATTRIB: 
 *	clear shell_surface flag to have TYPE_VIDEO_SURFACETEXTURE window
 *
 * shell_surface attribute mask as follows: 1th bit represent
 * has_status_bar, default value 0 means no status bar 2nd bit represent
 * show_status_bar, default value 0 means hide status bar 3rd bit represent
 * weston_resize_when_need_update_layout, default value 0 means client
 * resize
 */
enum wl_shell_surface_attribute_mask {
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_HAS_STATUS_BAR = 1,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_SHOW_STATUS_BAR = 2,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_WESTON_RESIZE_WHEN_NEED_UPDATE_LAYOUT = 4,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_SET_TYPE_GUEST_SURFACETEXTURE = 8,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_SET_TYPE_VIDEO_SURFACETEXTURE = 8,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_GUEST_SURFACETEXTURE_ATTRIB_ABOVE = 16,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_VIDEO_SURFACETEXTURE_ATTRIB_ABOVE = 16,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_GUEST_SURFACETEXTURE_ATTRIB_BELOW = 32,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_NEED_VIDEO_SURFACETEXTURE_ATTRIB_BELOW = 32,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_CLEAR_GUEST_SURFACETEXTURE_ATTRIB = 64,
	WL_SHELL_SURFACE_ATTRIBUTE_MASK_CLEAR_VIDEO_SURFACETEXTURE_ATTRIB = 64,
};
#endif /* WL_SHELL_SURFACE_ATTRIBUTE_MASK_ENUM */

#ifndef WL_SHELL_SURFACE_CONFIGURE_TYPE_ENUM
#define WL_SHELL_SURFACE_CONFIGURE_TYPE_ENUM
enum wl_shell_surface_configure_type {
	WL_SHELL_SURFACE_CONFIGURE_TYPE_INPUT_PANNEL_SHOW = 0,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_INPUT_PANNEL_HIDE = 1,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_ORIENTATION_CHANGED = 2,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_NAVIGATIONBAR_SHOW = 3,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_NAVIGATIONBAR_HIDE = 4,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_STATUSBAR_SHOW_TILE = 5,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_INPUT_PANEL_UPDATE = 6,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_CARD_SIZE_CHANGED = 7,
	WL_SHELL_SURFACE_CONFIGURE_TYPE_FULLSCREEN_OVERLAY = 8,
};
#endif /* WL_SHELL_SURFACE_CONFIGURE_TYPE_ENUM */

#ifndef WL_SHELL_SURFACE_ORIENTATION_ENUM
#define WL_SHELL_SURFACE_ORIENTATION_ENUM
enum wl_shell_surface_orientation {
	WL_SHELL_SURFACE_ORIENTATION_0 = 0,
	WL_SHELL_SURFACE_ORIENTATION_90 = 1,
	WL_SHELL_SURFACE_ORIENTATION_180 = 2,
	WL_SHELL_SURFACE_ORIENTATION_270 = 3,
};
#endif /* WL_SHELL_SURFACE_ORIENTATION_ENUM */
 ...
 * @set_visibility_event: set visibility event
 * @configure_change: window manager notify wms configure changed.
 *
...
struct wl_shell_surface_listener {
	 ...
	 * @isvisible: (none)
	 *
	 * The set visibility event asks the client to show or hide its
	 * surface.
	 */
	void (*set_visibility_event)(void *data,
				     struct wl_shell_surface *wl_shell_surface,
				     int32_t isvisible);
	/**
	 * configure_change - window manager notify wms configure
	 *	changed.
	 * @type: (none)
	 * @x: (none)
	 * @y: (none)
	 ...
	 * @params: (none)
	 *
	 * This configure event notify clients wms layout has changed.
	 * Type inform client which configure changed type event happend.
	 * The width and height arguments specify the size of the window in
	 * surface local coordinates. Orientation can be used while window
	 * orientation changed.
	 */
	void (*configure_change)(void *data,
				 struct wl_shell_surface *wl_shell_surface,
				 uint32_t type,
				 int32_t x,
				 int32_t y,
...
#define WL_SHELL_SURFACE_SET_GUEST	4
#define WL_SHELL_SURFACE_SET_TRANSIENT	5
#define WL_SHELL_SURFACE_SET_FULLSCREEN	6
#define WL_SHELL_SURFACE_SET_POPUP	7
#define WL_SHELL_SURFACE_SET_MAXIMIZED	8
#define WL_SHELL_SURFACE_SET_TITLE	9
#define WL_SHELL_SURFACE_SET_CLASS	10
#define WL_SHELL_SURFACE_SET_ZVALUE	11
#define WL_SHELL_SURFACE_SET_VISIBILITY	12
#define WL_SHELL_SURFACE_SET_WINDOW_GEOMETRY	13
#define WL_SHELL_SURFACE_SET_WINDOW_GEOMETRY_MODE	14
#define WL_SHELL_SURFACE_SET_MATRIX	15
#define WL_SHELL_SURFACE_SET_ALPHA	16
#define WL_SHELL_SURFACE_SET_ATTRIBUTE	17
#define WL_SHELL_SURFACE_SET_LAYOUT_FLAGS	18
#define WL_SHELL_SURFACE_SET_LAYOUT_MODE	19
#define WL_SHELL_SURFACE_SET_ZOFFSET	20
#define WL_SHELL_SURFACE_SET_VISIBLE_REGION_FLAG	21
#define WL_SHELL_SURFACE_SET_PARAMFLAGS	22
#define WL_SHELL_SURFACE_SET_CONFIGURE_CHANGED	23
...
static inline void
wl_shell_surface_set_guest(struct wl_shell_surface *wl_shell_surface)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_GUEST);
}
...
static inline void
wl_shell_surface_set_zvalue(struct wl_shell_surface *wl_shell_surface, uint32_t zvalue)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_ZVALUE, zvalue);
}

static inline void
wl_shell_surface_set_visibility(struct wl_shell_surface *wl_shell_surface, int32_t is_visible)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_VISIBILITY, is_visible);
}

static inline void
wl_shell_surface_set_window_geometry(struct wl_shell_surface *wl_shell_surface, int32_t x, int32_t y, int32_t w, int32_t h)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_WINDOW_GEOMETRY, x, y, w, h);
}

static inline void
wl_shell_surface_set_window_geometry_mode(struct wl_shell_surface *wl_shell_surface, int32_t mode)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_WINDOW_GEOMETRY_MODE, mode);
}

static inline void
wl_shell_surface_set_matrix(struct wl_shell_surface *wl_shell_surface, int32_t w, int32_t h, uint32_t type, struct wl_array *matrix)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_MATRIX, w, h, type, matrix);
}

static inline void
wl_shell_surface_set_alpha(struct wl_shell_surface *wl_shell_surface, uint32_t alpha)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_ALPHA, alpha);
}

static inline void
wl_shell_surface_set_attribute(struct wl_shell_surface *wl_shell_surface, int32_t attribute)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_ATTRIBUTE, attribute);
}

static inline void
wl_shell_surface_set_layout_flags(struct wl_shell_surface *wl_shell_surface, uint32_t layoutflags)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_LAYOUT_FLAGS, layoutflags);
}

static inline void
wl_shell_surface_set_layout_mode(struct wl_shell_surface *wl_shell_surface, uint32_t layoutmode)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_LAYOUT_MODE, layoutmode);
}

static inline void
wl_shell_surface_set_zoffset(struct wl_shell_surface *wl_shell_surface, uint32_t zoffset)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_ZOFFSET, zoffset);
}

static inline void
wl_shell_surface_set_visible_region_flag(struct wl_shell_surface *wl_shell_surface, uint32_t is_visible_region_null)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_VISIBLE_REGION_FLAG, is_visible_region_null);
}

static inline void
wl_shell_surface_set_paramflags(struct wl_shell_surface *wl_shell_surface, uint32_t paramflagtype, uint32_t paramflagvalue)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_PARAMFLAGS, paramflagtype, paramflagvalue);
}

static inline void
wl_shell_surface_set_configure_changed(struct wl_shell_surface *wl_shell_surface)
{
	wl_proxy_marshal((struct wl_proxy *) wl_shell_surface,
			 WL_SHELL_SURFACE_SET_CONFIGURE_CHANGED);
}
 ...
 * @notify_destroy: WindowManager notify client to destroy wl_surface
 *
...
struct wl_surface_listener {
	...
	void (*leave)(void *data,
		      ...
		      struct wl_output *output);
	/**
	 * notify_destroy - WindowManager notify client to destroy
	 *	wl_surface
	 *
	 * When WindowManager try to destroy wl_surface in server, it
	 * will notify client to call wl_surface_destroy
	 * (wl_shell_surface_destroy if any shell_surface bound with
	 * wl_surface in client) to release wl_surface in client side.
	 * Client app must listen this event in case it miss the destroy
	 * notification from WindowManager , which may cause unexpected
	 * result.
	 */
	void (*notify_destroy)(void *data,
			       struct wl_surface *wl_surface);
};
...
#define WL_SURFACE_ATTACH_FENCE	9
#define WL_SURFACE_FRAME_FENCE	10
#define WL_SURFACE_SET_NAME	11
#define WL_SURFACE_SET_TRANSPARENT_REGION	12
#define WL_SURFACE_SET_BEHIND_WINDOWS_BLUR_REGION	13
#define WL_SURFACE_SET_BEHIND_WINDOWS_BLUR_EFFECT	14
#define WL_SURFACE_SET_VIEWPORT_MODE	15
#define WL_SURFACE_SET_FIRSTDRAW_SYNC	16
...
static inline void
wl_surface_attach_fence(struct wl_surface *wl_surface, struct wl_buffer *buffer, int32_t x, int32_t y, int32_t fd)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_ATTACH_FENCE, buffer, x, y, fd);
}

static inline struct wl_callback *
wl_surface_frame_fence(struct wl_surface *wl_surface)
{
	struct wl_proxy *callback;

	callback = wl_proxy_marshal_constructor((struct wl_proxy *) wl_surface,
			 WL_SURFACE_FRAME_FENCE, &wl_callback_interface, NULL);

	return (struct wl_callback *) callback;
}

static inline void
wl_surface_set_name(struct wl_surface *wl_surface, const char *name)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_NAME, name);
}

static inline void
wl_surface_set_transparent_region(struct wl_surface *wl_surface, struct wl_region *region)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_TRANSPARENT_REGION, region);
}

static inline void
wl_surface_set_behind_windows_blur_region(struct wl_surface *wl_surface, int32_t x, int32_t y, int32_t width, int32_t height, int32_t is_relative_coordinates_for_blur)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_BEHIND_WINDOWS_BLUR_REGION, x, y, width, height, is_relative_coordinates_for_blur);
}

static inline void
wl_surface_set_behind_windows_blur_effect(struct wl_surface *wl_surface, int32_t downsamplefactor, int32_t kernelsize)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_BEHIND_WINDOWS_BLUR_EFFECT, downsamplefactor, kernelsize);
}

static inline void
wl_surface_set_viewport_mode(struct wl_surface *wl_surface, int32_t viewport_mode)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_VIEWPORT_MODE, viewport_mode);
}

static inline void
wl_surface_set_firstdraw_sync(struct wl_surface *wl_surface, int32_t is_sync)
{
	wl_proxy_marshal((struct wl_proxy *) wl_surface,
			 WL_SURFACE_SET_FIRSTDRAW_SYNC, is_sync);
}
 ...
 * @WL_SEAT_CAPABILITY_JOYSTICK: The seat has joystick devices
 * @WL_SEAT_CAPABILITY_VIBRATOR: The seat has vibrator devices
 *
...
enum wl_seat_capability {
	...
	WL_SEAT_CAPABILITY_JOYSTICK = 8,
	WL_SEAT_CAPABILITY_VIBRATOR = 16,
};
#endif /* WL_SEAT_CAPABILITY_ENUM */

#ifndef WL_SEAT_DEVICE_ACTION_ENUM
#define WL_SEAT_DEVICE_ACTION_ENUM
/**
 * wl_seat_device_action - device change action
 * @WL_SEAT_DEVICE_ACTION_ADD: device is add
 * @WL_SEAT_DEVICE_ACTION_REMOVE: device is remove
 * @WL_SEAT_DEVICE_ACTION_CHANGE: device is change
 *
 * Describes the action of a device change
 */
enum wl_seat_device_action {
	WL_SEAT_DEVICE_ACTION_ADD = 0,
	WL_SEAT_DEVICE_ACTION_REMOVE = 1,
	WL_SEAT_DEVICE_ACTION_CHANGE = 2,
};
#endif /* WL_SEAT_DEVICE_ACTION_ENUM */

#ifndef WL_SEAT_MODE_ENUM
#define WL_SEAT_MODE_ENUM
/**
 * wl_seat_mode - seat mode bitmask
 * @WL_SEAT_MODE_TABLET: The seat is tablet mode
 * @WL_SEAT_MODE_MOVIE: The seat is movie mode
 * @WL_SEAT_MODE_CLAMSHELL: The seat is clamshell mode
 *
 * This is a bitmask of mode this seat has; if a member is set, then it
 * is present on the seat.
 */
enum wl_seat_mode {
	WL_SEAT_MODE_TABLET = 0,
	WL_SEAT_MODE_MOVIE = 1,
	WL_SEAT_MODE_CLAMSHELL = 3,
};
#endif /* WL_SEAT_MODE_ENUM */

/**
 * wl_seat - group of input devices
 * @capabilities: seat capabilities changed
 * @name: unique identifier for this seat
 * @mode_change: seat mode changed
 *
...
struct wl_seat_listener {
	 ...
	 * @mode: (none)
	 *
	 * This is emitted whenever a seat has mode changed.
	 */
	void (*mode_change)(void *data,
			    struct wl_seat *wl_seat,
			    uint32_t mode);
};
...
static inline struct wl_vibrator *
wl_seat_get_vibrator(struct wl_seat *wl_seat)
{
	struct wl_proxy *id;

	id = wl_proxy_marshal_constructor((struct wl_proxy *) wl_seat,
			 WL_SEAT_GET_VIBRATOR, &wl_vibrator_interface, NULL);

	return (struct wl_vibrator *) id;
}
...
#define WL_POINTER_SET_VISIBILITY	2
#define WL_POINTER_SET_TYPE	3
...
static inline void
wl_pointer_set_visibility(struct wl_pointer *wl_pointer, uint32_t visibility)
{
	wl_proxy_marshal((struct wl_proxy *) wl_pointer,
			 WL_POINTER_SET_VISIBILITY, visibility);
}

static inline void
wl_pointer_set_type(struct wl_pointer *wl_pointer, uint32_t type)
{
	wl_proxy_marshal((struct wl_proxy *) wl_pointer,
			 WL_POINTER_SET_TYPE, type);
}
 ...
 * @WL_KEYBOARD_KEY_STATE_REPEATED: key is repeated
 * @WL_KEYBOARD_KEY_STATE_LONGPRESSED: key is long pressed
 * @WL_KEYBOARD_KEY_STATE_CANCEL: key is canceled
 *
...
enum wl_keyboard_key_state {
	...
	WL_KEYBOARD_KEY_STATE_LONGPRESSED = 3,
	WL_KEYBOARD_KEY_STATE_CANCEL = 4,
};
#endif /* WL_KEYBOARD_KEY_STATE_ENUM */

#ifndef WL_KEYBOARD_EVENT_HANDLE_STATE_ENUM
#define WL_KEYBOARD_EVENT_HANDLE_STATE_ENUM
/**
 * wl_keyboard_event_handle_state - event handle state
 * @WL_KEYBOARD_EVENT_HANDLE_STATE_UNHANDLED: event is not handled
 * @WL_KEYBOARD_EVENT_HANDLE_STATE_HANDLED: event is handled
 *
 * Describes the event handle state, handled, unhandled. etc
 */
enum wl_keyboard_event_handle_state {
	WL_KEYBOARD_EVENT_HANDLE_STATE_UNHANDLED = 0,
	WL_KEYBOARD_EVENT_HANDLE_STATE_HANDLED = 1,
};
#endif /* WL_KEYBOARD_EVENT_HANDLE_STATE_ENUM */
...
#define WL_KEYBOARD_REGISTER_KEY_LISTENER	1
#define WL_KEYBOARD_UNREGISTER_KEY_LISTENER	2
#define WL_KEYBOARD_EVENT_HANDLED	3
...
static inline void
wl_keyboard_register_key_listener(struct wl_keyboard *wl_keyboard)
{
	wl_proxy_marshal((struct wl_proxy *) wl_keyboard,
			 WL_KEYBOARD_REGISTER_KEY_LISTENER);
}

static inline void
wl_keyboard_unregister_key_listener(struct wl_keyboard *wl_keyboard)
{
	wl_proxy_marshal((struct wl_proxy *) wl_keyboard,
			 WL_KEYBOARD_UNREGISTER_KEY_LISTENER);
}

static inline void
wl_keyboard_event_handled(struct wl_keyboard *wl_keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t key_state, uint32_t event_state)
{
	wl_proxy_marshal((struct wl_proxy *) wl_keyboard,
			 WL_KEYBOARD_EVENT_HANDLED, serial, time, key, key_state, event_state);
}

#ifndef WL_TOUCH_DEVICE_ID_ENUM
#define WL_TOUCH_DEVICE_ID_ENUM
/**
 * wl_touch_device_id - touch device id
 * @WL_TOUCH_DEVICE_ID_TOUCH_SCREEN: touch screen
 * @WL_TOUCH_DEVICE_ID_TOUCH_PAD: touch pad
 *
 * Describes touch device id
 */
enum wl_touch_device_id {
	WL_TOUCH_DEVICE_ID_TOUCH_SCREEN = 0,
	WL_TOUCH_DEVICE_ID_TOUCH_PAD = 1,
};
#endif /* WL_TOUCH_DEVICE_ID_ENUM */
...
struct wl_touch_listener {
	 ...
	 * @time: timestamp with millisecond granularity
	 * @surface: (none)
	 * @device_id: the device ID of this touch point
	 ...
	 * @sx: x coordinate in screen coordinates
	 * @sy: y coordinate in screen coordinates
	 *
	 ...
	 * @sx: x coordinate in screen coordinates
	 * @sy: y coordinate in screen coordinates
	 * @positions: the reserved positions
	 *
...
#define WL_TOUCH_EVENT_HANDLED	1
...
static inline void
wl_touch_event_handled(struct wl_touch *wl_touch, uint32_t serial, uint32_t time, uint32_t state)
{
	wl_proxy_marshal((struct wl_proxy *) wl_touch,
			 WL_TOUCH_EVENT_HANDLED, serial, time, state);
}

#define WL_VIBRATOR_START_VIBRATE	0
#define WL_VIBRATOR_STOP_VIBRATE	1
#define WL_VIBRATOR_RELEASE	2

static inline void
wl_vibrator_set_user_data(struct wl_vibrator *wl_vibrator, void *user_data)
{
	wl_proxy_set_user_data((struct wl_proxy *) wl_vibrator, user_data);
}

static inline void *
wl_vibrator_get_user_data(struct wl_vibrator *wl_vibrator)
{
	return wl_proxy_get_user_data((struct wl_proxy *) wl_vibrator);
}

static inline void
wl_vibrator_destroy(struct wl_vibrator *wl_vibrator)
{
	wl_proxy_destroy((struct wl_proxy *) wl_vibrator);
}

static inline void
wl_vibrator_start_vibrate(struct wl_vibrator *wl_vibrator, uint32_t id)
{
	wl_proxy_marshal((struct wl_proxy *) wl_vibrator,
			 WL_VIBRATOR_START_VIBRATE, id);
}

static inline void
wl_vibrator_stop_vibrate(struct wl_vibrator *wl_vibrator, uint32_t id)
{
	wl_proxy_marshal((struct wl_proxy *) wl_vibrator,
			 WL_VIBRATOR_STOP_VIBRATE, id);
}

static inline void
wl_vibrator_release(struct wl_vibrator *wl_vibrator)
{
	wl_proxy_marshal((struct wl_proxy *) wl_vibrator,
			 WL_VIBRATOR_RELEASE);
...
#define WL_SUBSURFACE_SET_VISIBILITY	6
#define WL_SUBSURFACE_SET_DEPENDENT_VISIBILITY	7
...
static inline void
wl_subsurface_set_visibility(struct wl_subsurface *wl_subsurface, int32_t is_visible)
{
	wl_proxy_marshal((struct wl_proxy *) wl_subsurface,
			 WL_SUBSURFACE_SET_VISIBILITY, is_visible);
}

static inline void
wl_subsurface_set_dependent_visibility(struct wl_subsurface *wl_subsurface, int32_t is_dependent)
{
	wl_proxy_marshal((struct wl_proxy *) wl_subsurface,
			 WL_SUBSURFACE_SET_DEPENDENT_VISIBILITY, is_dependent);
}
