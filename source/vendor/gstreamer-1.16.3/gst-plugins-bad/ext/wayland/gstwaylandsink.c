...
static gboolean
gst_waylandsink_fix_video_info (GstWaylandSink * sink,
    GstVideoInfo * video_info)
{
  gboolean ret = TRUE;

  if (!sink->is_will_crop)
    return FALSE;

  video_info->width = sink->out_crop.width;
  video_info->height = sink->out_crop.height;
...
gst_wayland_sink_set_caps (GstBaseSink * bsink, GstCaps * caps)
{
  ...
  if (gst_structure_get_boolean (s, "crop", &isCrop) && isCrop) {
    if (gst_structure_get_uint (s, "crop_x", &sink->out_crop.x) &&
        gst_structure_get_uint (s, "crop_y", &sink->out_crop.y) &&
        gst_structure_get_uint (s, "crop_width", &sink->out_crop.width) &&
        gst_structure_get_uint (s, "crop_height", &sink->out_crop.height)) {
      sink->is_will_crop = TRUE;
      gst_waylandsink_fix_video_info (sink, &sink->video_info);
      GST_DEBUG_OBJECT (sink,
          "crop_x:%u, crop_y:%u, crop_width:%u, crop_height:%u",
          sink->out_crop.x, sink->out_crop.y, sink->out_crop.width,
          sink->out_crop.height);
    } else {
      isCrop = FALSE;
      sink->is_will_crop = FALSE;
      GST_INFO_OBJECT (sink, "error crop info, please check caps");
    }
  }
