...
typedef struct{
    ...
    int32_t padded_y_stride;
    int32_t padded_c_stride;

    int32_t padded_y_height;
    int32_t padded_c_height;

    int32_t enc_stride;
    int32_t enc_width;
    int32_t enc_height;

    int32_t enc_c_stride;
    ...
    )
{
    ...
    int32_t     ystride;
    int32_t     out_frm_format;
    Context*    ctx = (Context*)impl->context;

    uint8_t*    pYuv_int8 = ctx->pYuv;
    osal_file_t yuvFp = ctx->fp;
    uint16_t*   pYuv_int16 = NULL, *pYuv2_int16 = NULL;
    int32_t     outsize, out_width, out_height, pxl_format, yuv3p4b = 0;
    int32_t     bitdepth = 0;
    uint32_t    i;
    ...
    if ((pYuv_int16 = (uint16_t*)osal_malloc(outsize * sizeof(uint16_t))) == NULL) {
        printf("malloc error: outbuf\n");
        return 0;
    }
    if ((pYuv2_int16 = (uint16_t*)osal_malloc(outsize * sizeof(uint16_t))) == NULL) {
        printf("malloc error: outbuf\n");
        return 0;
    }
        ...
        if ( yuv3p4b == 0 )
        {
            ...
            **************************************************************/
            for (i=0; i<srcFrameSize/2; i++) { 
                pYuv_int8[i*2] = (uint8_t)(pYuv_int16[i]>>8)&0xff;    
                pYuv_int8[i*2 + 1] = (uint8_t)(pYuv_int16[i])&0xff;
            }
#if 0
            /***************************************************************
            * when 10bit source YUV == BIG ENDIAN, 16LSB
            **************************************************************/
            memcpy(pYuv_int8, pYuv_int16, srcFrameSize);
#endif
        }
        else
        {
