...
int parseAvcCfgFile(ENC_CFG *pEncCfg, char *FileName)
{
    ...
    */
    ...
    else
    {
        ...
        if (pEncCfg->RcMBInterval<=0 || pEncCfg->RcMBInterval >64)
            pEncCfg->RcMBInterval = 1;
    }
    ...
    memset(pEncCfg->skipPicNums, 0, sizeof(pEncCfg->skipPicNums));
    ...
    )
{
    ...
    memset(gopPicParam, 0, sizeof(CustomGopPicParam));
    *gopPicLambda = 0;
    ...
    )
{
    ...
    memset(roiRegion, 0, sizeof(VpuRect));
    *roiLevel = 0;
...

{
    ...
    int intra8=0, intra16=0, intra32=0, frameSkip=0, numTemporalLayers=0, dynamicMergeEnable; // temp value
    UNREFERENCED_PARAMETER(frameSkip);
    UNREFERENCED_PARAMETER(numTemporalLayers);
    ...
    if (HEVC_GetValue(fp, NUM_TEMPORAL_LAYERS, &iValue) == 0)
        goto __end_parseHevcCfgFile;
    else
        numTemporalLayers = iValue;
#if TEMP_SCALABLE_RC
    numTemporalLayers = 1;
#endif
...
#if TEMP_SCALABLE_RC
            if (pEncCfg->hevcCfg.gopParam.picParam[i].temporalId >= numTemporalLayers) {
                numTemporalLayers = pEncCfg->hevcCfg.gopParam.picParam[i].temporalId + 1;
            }
            if (numTemporalLayers > MAX_NUM_TEMPORAL_LAYER) {
                VLOG(ERR, "CFG file error : %s MaxTempLayer %d exceeds MAX_TEMP_LAYER(7). \n", tempStr, numTemporalLayers);
                goto __end_parseHevcCfgFile;
            }
#endif
        }
