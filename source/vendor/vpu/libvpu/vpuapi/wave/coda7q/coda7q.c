...
RetCode Coda7qVpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    if (result->sequenceChanged) {
        pDecInfo->scaleWidth  = pDecInfo->initialInfo.picWidth;
        pDecInfo->scaleHeight = pDecInfo->initialInfo.picHeight;
    }
...
extern RetCode Coda7qVpuEncSetup(CodecInst* instance)
{
    ...
    pEncInfo->vbTemp[0].size      = 0;
    pEncInfo->vbTemp[1].size      = 0;
#else
...
RetCode Coda7qVpuEncode(CodecInst* instance, EncParam* option)
{
    ...
    /* Set up temp-buffer */
#if defined(WAVE420SN) || defined(WAVE420DU)
    VpuWriteReg(coreIdx, W4_ADDR_TEMP_BASE, pEncInfo->vbTemp[0].phys_addr);
    VpuWriteReg(coreIdx, W4_TEMP_SIZE,      pEncInfo->vbTemp[0].size);
#else
