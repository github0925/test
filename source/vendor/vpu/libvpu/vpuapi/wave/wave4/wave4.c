...
{
    RetCode ret;
    uint32_t    modeOption = DEC_PIC_NORMAL;
	...
	VpuWriteReg(instance->coreIdx, W4_CMD_DEC_PAD_PIXEL, (option->pixelLumaPad<<10 | option->pixelChromaPad));
...
RetCode Wave4VpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    result->h265Info.vuiParamPresent    = 0;
    result->h265Info.colorDescPresent   = 0;
    result->h265Info.masterMetaPresent  = 0;
    result->h265Info.contentMetaPresent = 0;
    if (result->decOutputExtData.userDataHeader != 0) {
        Uint8* pBase = (Uint8*)osal_malloc(H265_USERDATA_BUFFER_SIZE);
        if (pBase != NULL) {
            user_data_entry_t* pEntry = (user_data_entry_t*)pBase;
            VpuReadMem(instance->coreIdx, pDecInfo->userDataBufAddr, pBase,  pDecInfo->userDataBufSize, VPU_USER_DATA_ENDIAN);

            // get the vui info
            if (result->decOutputExtData.userDataHeader & (1<<H265_USERDATA_FLAG_VUI)) {
                h265_vui_param_t*  vui = (h265_vui_param_t*)(pBase + pEntry[H265_USERDATA_FLAG_VUI].offset);
                result->h265Info.vuiParamPresent = 1;
                result->h265Info.colorDescPresent = vui->colour_description_present_flag;
                result->h265Info.vidFullRange = vui->video_full_range_flag;
                result->h265Info.colorPrimaries = vui->colour_primaries;
                result->h265Info.vuiMatrixCoefficients = vui->matrix_coefficients;
                result->h265Info.vuiTransferCharacteristics = vui->transfer_characteristics;
            }

            if (result->decOutputExtData.userDataHeader & (1<<H265_USERDATA_FLAG_MASTERING_COLOR_VOL)) {
                h265_mastering_display_colour_volume_t* mastering =
                    (h265_mastering_display_colour_volume_t*)(pBase + pEntry[H265_USERDATA_FLAG_MASTERING_COLOR_VOL].offset);
                VLOG(INFO, " WHITE_POINT_X: 0x%x WHITE_POINT_Y: 0x%x\n", mastering->white_point_x, mastering->white_point_y);
                osal_memcpy((Uint8*)&result->h265Info.display_primaries_x, (Uint8*)mastering, sizeof(h265_mastering_display_colour_volume_t));
                result->h265Info.masterMetaPresent = 1;
            }

            if (result->decOutputExtData.userDataHeader & (1<<H265_USERDATA_FLAG_CONTENT_LIGHT_LEVEL_INFO)) {
                content_light_level_info_t* content_light_level =
                    (content_light_level_info_t*)(pBase + pEntry[H265_USERDATA_FLAG_CONTENT_LIGHT_LEVEL_INFO].offset);
                VLOG(INFO, " max_content_light_level: 0x%x max_pic_average_light_level: 0x%x\n",
                    content_light_level->max_content_light_level, content_light_level->max_pic_average_light_level);
                result->h265Info.max_content_light_level = content_light_level->max_content_light_level;
                result->h265Info.max_pic_average_light_level = content_light_level->max_pic_average_light_level;
                result->h265Info.contentMetaPresent = 1;
            }
...
RetCode Wave4VpuDecGetSeqInfo(CodecInst* pCodecInst, DecInitialInfo* info)
{
    ...
    if (pCodecInst->productId == PRODUCT_ID_4102) {
        pDecInfo->scaleWidth  = info->picWidth;
        pDecInfo->scaleHeight = info->picHeight;
    }
    else if (pCodecInst->productId == PRODUCT_ID_412) {
...
RetCode Wave4VpuEncRegisterFramebuffer(CodecInst* instance, FrameBuffer* fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    if (mapType == COMPRESSED_FRAME_MAP) {
        ...
        if (instance->productId == PRODUCT_ID_420SN) {
    ...
    if ((pEncInfo->rotationAngle != 0 || pEncInfo->mirrorDirection != 0) && !(pEncInfo->rotationAngle == 180 && pEncInfo->mirrorDirection == MIRDIR_HOR_VER)){
        if (instance->productId == PRODUCT_ID_420SN) {
            lumaStride  = VPU_ALIGN16(VPU_ALIGN32(bufWidth) + 16)*32;
            chromaStride= VPU_ALIGN16(VPU_ALIGN32(bufWidth)/2 + 16)*32;
        ...
        if (instance->productId == PRODUCT_ID_420SN) {
            lumaStride  = VPU_ALIGN16(pOpenParam->picWidth + 16) * 32;
            chromaStride= VPU_ALIGN16(pOpenParam->picWidth/2 + 16) * 32;
        }
        else {
    ...
    if (instance->productId == PRODUCT_ID_420SN || instance->productId == PRODUCT_ID_420DU) {
        VpuWriteReg(coreIdx, W4_CMD_FBC_STRIDE_Y, lumaStride);
        VpuWriteReg(coreIdx, W4_CMD_FBC_STRIDE_C, chromaStride);
    }
    else {
    ...
    if (ConfigSecAXIWave(coreIdx, pOpenParam->bitstreamFormat,
...
RetCode Wave4VpuEncSetup(CodecInst* instance)
{
    ...
    pEncInfo->vbTemp[0].phys_addr = vb.phys_addr + WAVE4_TEMPBUF_OFFSET;
    pEncInfo->vbTemp[0].virt_addr = vb.virt_addr + WAVE4_TEMPBUF_OFFSET;
    pEncInfo->vbTemp[0].size      = DEFAULT_TEMPBUF_SIZE;
    pEncInfo->vbTemp[1].phys_addr = vb.phys_addr + WAVE4_TEMPBUF_OFFSET + DEFAULT_TEMPBUF_SIZE;
    pEncInfo->vbTemp[1].virt_addr = vb.virt_addr + WAVE4_TEMPBUF_OFFSET + DEFAULT_TEMPBUF_SIZE;
    pEncInfo->vbTemp[1].size      = DEFAULT_TEMPBUF_SIZE;

    VpuWriteReg(coreIdx, W4_ADDR_TEMP_BASE, pEncInfo->vbTemp[pEncInfo->vbTempQueueIndex].phys_addr);
    VpuWriteReg(coreIdx, W4_TEMP_SIZE,      pEncInfo->vbTemp[pEncInfo->vbTempQueueIndex].size);
#else
    ...
    if (instance->productId == PRODUCT_ID_420 || instance->productId == PRODUCT_ID_420SN || instance->productId == PRODUCT_ID_420DU) {
    ...
    if (instance->productId == PRODUCT_ID_420SN)
        pHevc->initBufLevelx8 = 0;      // wave420sn doesn't support initBufLevelx8.
    ...
    if (instance->productId == PRODUCT_ID_420DU || instance->productId == PRODUCT_ID_420SN) {
        VpuWriteReg(coreIdx, W4_CMD_ENC_RC_INIT_QP, (pHevc->initialRcQp&0xFF));
    }
    else {
...
RetCode Wave4VpuEncGetResult(CodecInst* instance, EncOutputInfo* result)
{
...
#if defined(WAVE420SN) || defined(WAVE420DU)
    result->feedingSignal = (regVal>>5)&0x3;
#endif
...
#if defined(WAVE420SN) || defined(WAVE420DU)
    if (instance->instIndex == result->encInstIdx) {
#endif
        pEncInfo->streamWrPtr   = VpuReadReg(coreIdx, pEncInfo->streamWrPtrRegAddr);
#if defined(WAVE420SN) || defined(WAVE420DU)
    }
    ...
    result->bitstreamBuffer = VpuReadReg(coreIdx, pEncInfo->streamRdPtrRegAddr);
    result->rdPtr = VpuReadReg(coreIdx, pEncInfo->streamRdPtrRegAddr);
    result->wrPtr = VpuReadReg(coreIdx, pEncInfo->streamWrPtrRegAddr);
//  VLOG(INFO, "%d, %d, (%x, %x), (%x, %x), picCnt=%d\n", instance->instIndex, result->encInstIdx,
//        pEncInfo->streamRdPtr, pEncInfo->streamWrPtr, result->rdPtr, result->wrPtr, result->encPicCnt);
#else
    ...
    if (param->changeParaMode == OPT_COMMON) {
        ...
        if (param->enable_option & ENC_PARAM_CHANGE) {
            VpuWriteReg(coreIdx, W4_CMD_ENC_PARAM,  (param->useRecommendEncParam)           |
                                                    ...
                                                    (param->betaOffsetDiv2<<17)             |
                                                    (param->tcOffsetDiv2<<21)               |
                                                    (param->skipIntraTrans<<25)             |
                                                    (param->saoEnable<<26)                  |
                                                    (param->intraInInterSliceEnable<<27)    |
                                                    (param->intraNxNEnable<<28));
        }
        if (param->enable_option & ENC_RC_INIT_QP) {
            if (instance->productId == PRODUCT_ID_420SN || instance->productId == PRODUCT_ID_420DU){
                VpuWriteReg(coreIdx, W4_CMD_ENC_RC_INIT_QP, (param->initialRcQp&0xFF));
            }
        }
...
RetCode CheckEncCommonParamValid(EncOpenParam* pop)
{
    ...
    if( !low_delay && (param->intraPeriod != 0) && ((param->intraPeriod % intra_period_gop_step_size) != 0) ) {
        APIDPRINT("CFG FAIL : Not support intra period[%d] for the gop structure\n", param->intraPeriod);
        APIDPRINT("RECOMMEND CONFIG PARAMETER : Intra period = %d\n", intra_period_gop_step_size * (param->intraPeriod / intra_period_gop_step_size));
        ret = RETCODE_FAILURE;
    }

    if( !low_delay && (param->intraPeriod != 0) && ((param->intraPeriod % intra_period_gop_step_size) == 1) && param->decodingRefreshType == 0)
    {
        APIDPRINT("CFG FAIL : Not support decoding refresh type[%d] for closed gop structure\n", param->decodingRefreshType );
        APIDPRINT("RECOMMEND CONFIG PARAMETER : Decoding refresh type = IDR\n");
        ret = RETCODE_FAILURE;
    }
    ...
    if (param->gopPresetIdx == PRESET_IDX_CUSTOM_GOP) {
        for(i = 0; i < param->gopParam.customGopSize; i++)
        {
            ...
            {
                APIDPRINT("CFG FAIL : temporalId %d exceeds MAX_NUM_TEMPORAL_LAYER\n", param->gopParam.picParam[i].temporalId);
                APIDPRINT("RECOMMEND CONFIG PARAMETER : Adjust temporal ID under MAX_NUM_TEMPORAL_LAYER(7) in GOP structure\n");
                ret = RETCODE_FAILURE;
            }

            if (param->gopParam.picParam[i].temporalId < 0)
            {
                APIDPRINT("CFG FAIL : Must be %d-th temporal_id >= 0\n",param->gopParam.picParam[i].temporalId);
                APIDPRINT("RECOMMEND CONFIG PARAMETER : Adjust temporal layer above '0' in GOP structure\n");
                ret = RETCODE_FAILURE;
            }
        }
    }
    ...
    if (param->useRecommendEncParam == 0)
    {
        ...
        {
            APIDPRINT("CFG FAIL : If intraInInterSliceEnable is '0', Intra refresh mode must be '0'\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : intraRefreshMode = 0\n");
            ret = RETCODE_FAILURE;
        }
        ...
        // RDO
        {
            ...
            {
                APIDPRINT("All CU size mode should be enabled.\n");
                ret = RETCODE_FAILURE;
            }

            if( ((param->cuSizeMode&0x1) == 0) && ((align_8_width_flag != 0) || (align_8_height_flag != 0)) )
            {
                APIDPRINT("CFG FAIL : Picture width and height must be aligned with 8 pixels when enable CU8x8 of cuSizeMode \n");
                APIDPRINT("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x1 (CU8x8)\n");
                ret = RETCODE_FAILURE;
            }
            else if(((param->cuSizeMode&0x1) == 0) && ((param->cuSizeMode&0x2) == 0) && ((align_16_width_flag != 0) || (align_16_height_flag != 0)) )
            {
                APIDPRINT("CFG FAIL : Picture width and height must be aligned with 16 pixels when enable CU16x16 of cuSizeMode\n");
                APIDPRINT("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x2 (CU16x16)\n");
                ret = RETCODE_FAILURE;
            }
            ...
            {
                APIDPRINT("CFG FAIL : Picture width and height must be aligned with 32 pixels when enable CU32x32 of cuSizeMode\n");
                APIDPRINT("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x4 (CU32x32)\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        {
            APIDPRINT("CFG FAIL : If WaveFrontSynchro(WPP) '1', the option of multi-slice must be '0'\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : independSliceMode = 0, dependSliceMode = 0\n");
            ret = RETCODE_FAILURE;
        }
    }
    ...
    // Slice
    {
        ...
        {
            APIDPRINT("CFG FAIL : If independSliceMode is '0', dependSliceMode must be '0'\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : independSliceMode = 1, independSliceModeArg = TotalCtuNum\n");
            ret = RETCODE_FAILURE;
        }
        else if((param->independSliceMode == 1) && (param->dependSliceMode == 1) )
        {
            ...
            {
                APIDPRINT("CFG FAIL : If independSliceMode & dependSliceMode is both '1' (multi-slice with ctu count), must be independSliceModeArg >= dependSliceModeArg\n");
                APIDPRINT("RECOMMEND CONFIG PARAMETER : dependSliceMode = 0\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if (param->independSliceMode != 0)
        {
            ...
            {
                APIDPRINT("CFG FAIL : If independSliceMode is not 0, must be independSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if (param->dependSliceMode != 0)
        {
            ...
            {
                APIDPRINT("CFG FAIL : If dependSliceMode is not 0, must be dependSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
    }
    ...
    if (param->confWinTop % 2) {
        APIDPRINT("CFG FAIL : conf_win_top : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinTop);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinBot % 2) {
        APIDPRINT("CFG FAIL : conf_win_bot : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinBot);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinLeft % 2) {
        APIDPRINT("CFG FAIL : conf_win_left : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinLeft);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinRight % 2) {
        APIDPRINT("CFG FAIL : conf_win_right : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinRight);
        ret = RETCODE_FAILURE;
    }
...
RetCode CheckEncRcParamValid(EncOpenParam* pop)
{
    ...
    if(pop->rcEnable == 1)
    {
        ...
        {
            APIDPRINT("CFG FAIL : Not allowed bitRate > transRate\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : bitRate = transRate (CBR)\n");
            ret = RETCODE_FAILURE;
        }

        if(param->minQp > param->maxQp)
        {
            APIDPRINT("CFG FAIL : Not allowed MinQP > MaxQP\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : MinQP = MaxQP\n");
            ret = RETCODE_FAILURE;
        }

        if(pop->bitRate <= (int) pop->frameRateInfo)
        {
            APIDPRINT("CFG FAIL : Not allowed EncBitRate <= FrameRate\n");
            APIDPRINT("RECOMMEND CONFIG PARAMETER : EncBitRate = FrameRate * 10000\n");
            ret = RETCODE_FAILURE;
        }
    }
