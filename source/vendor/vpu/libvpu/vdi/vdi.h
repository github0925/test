...
// #define SUPPORT_MULTI_CORE_IN_ONE_DRIVER   ctm
 ...
 *@param virt_addr virt address in user space  when  summu enable
...
/**
 *@Brief Configuration  Line buffer & Sec AXI
 *@param MODE_SRAM_AXI_EXCLUSIVE2
 *                  Secondary AXI   168K
 *@param MODE_SRAM_AXI_LINEBUFFER_SHARE
 *                  line buffer     72K
 *                  Secondary AXI   96K
 *@param MODE_SRAM_LINEBUFFER
 *                  line buffer     144K
 *@param MODE_SRAM_MAX
 *                  Secondary AXI   168K
 */
typedef enum {
    MODE_SRAM_AXI_EXCLUSIVE,
    MODE_SRAM_AXI_LINEBUFFER_SHARE,
    MODE_SRAM_LINEBUFFER,
    MODE_SRAM_MAX
} SramMode;
...
typedef struct vpu_instance_pool_t {
    unsigned char
    codecInstPool[MAX_NUM_INSTANCE][MAX_INST_HANDLE_SIZE];  // Since VDI don't know the size of CodecInst structure, VDI should have the enough space not to overflow.
...
int vdi_release(unsigned long
                core_idx);    //this function may be called only at system off.
...
int vdi_set_change_clock(unsigned long core_idx,
                         unsigned long clock_mask);
...
int vdi_close_instance(unsigned long core_idx,
...
*@Brief: vdi_device_memory_map   Map device memory
        getting dma_addr should mmap device,
        component can use function to get dma adrress according
        dma_handle.
        dma address must attach one master device
*@param: core_idx coda988 or wave412
*@param: vdb description of memory info
*@return 0 success  -1 failure
*/
int vdi_device_memory_map(unsigned long core_idx, vpu_buffer_t *vb);
...
*@param: vdb description of memory info
*@return void
*@note:  the value of vdb-member should not been changed
*        by vdi_device_memory_map()-return
*/
void vdi_device_memory_unmap(unsigned long core_idx, vpu_buffer_t *vdb);
