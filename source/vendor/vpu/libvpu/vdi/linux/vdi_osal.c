...
#define DEBUG_LOG_API "debug.log.vpu.api"
static long s_debug_level = NONE;
#elif defined GSTREAMER_LOG
#include <gst/gst.h>
#include <gst/gstinfo.h>
#define DEBUG_LOG_API "gstomxvpuapi"
static GstDebugCategory * gst_omx_vpu_debug_category = NULL;
#endif
...
void LogMsg(int level, const char *format, ...)
{
    ...
    {
        if (s_debug_level > 1) //DEB_LEV_ERR
        {
            ALOGI("%s", logBuf);
        }
        else
    ...
    fputs(logBuf, stderr);
#elif defined GSTREAMER_LOG
    GST_DEBUG_CATEGORY_INIT (gst_omx_vpu_debug_category, DEBUG_LOG_API, 0, \
        "debug category for gst-omx-vpu video codec base class");
    if (level == ERR)
    {
        GST_CAT_ERROR(gst_omx_vpu_debug_category, "%s", logBuf);
    }
    else
    {
        GST_CAT_LOG(gst_omx_vpu_debug_category, "%s", logBuf);
    }
#else
    if (level == ERR)
    {
...
void DebugLevel() {
    ...
    if (property_get(DEBUG_LOG_API, value, NULL)) {
        char *end;
