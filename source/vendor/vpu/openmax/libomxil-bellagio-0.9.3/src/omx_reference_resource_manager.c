...
static OMX_BOOL hasActivedComponent()
{
    if (listOfcomponentRegistered) {
        int i = 0;
        while(listOfcomponentRegistered[i].component_name != NULL)
        {
            int index = listOfcomponentRegistered[i].index;
            if (numElemInList(globalComponentList[index]))
            {
                return OMX_TRUE;
            }
            i++;
...
OMX_ERRORTYPE preGetResource(int indexComponent)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;

    DEBUG(DEB_LEV_FUNCTION_NAME, "In %s\n", __func__);

    if (numElemInList(globalComponentList[indexComponent])
            >= listOfcomponentRegistered[indexComponent].max_components) {
        DEBUG(DEB_LEV_ERR, "In %s, exeed the num limit, return with error\n", __func__);
...
OMX_ERRORTYPE RM_RegisterComponent(char *name, int max_components) {
...
#ifdef ANDROID
            ret = preGetResource(listOfcomponentRegistered[i].index);
#endif
    ...
    resGetCorrespondConfig(name, &max_num);
    if( max_components < max_num) {
        max_num = max_components;
    }

    listOfcomponentRegistered[i].max_components = max_num;
    listOfcomponentRegistered[i].capacity = MAX_COMPONENTS_CAPACITY;
...
OMX_ERRORTYPE RM_Deinit() {
    ...
    if (listOfcomponentRegistered) {
        i = 0;
        while (listOfcomponentRegistered[i].component_name != NULL) {
            free(listOfcomponentRegistered[i].component_name);
            listOfcomponentRegistered[i].component_name = NULL;
            i++;
        }

        free(listOfcomponentRegistered);
        listOfcomponentRegistered = NULL;
    }

    if (globalWaitingComponentList) {
        free(globalWaitingComponentList);
        globalWaitingComponentList = NULL;
    }

    if (globalComponentList) {
        free(globalComponentList);
        globalComponentList = NULL;
    }
...
void updateResourceCapacity(float weight, int indexComponent, OMX_BOOL restore)
{
    if(restore == OMX_TRUE) {
        listOfcomponentRegistered[indexComponent].capacity += weight;
    } else {
        listOfcomponentRegistered[indexComponent].capacity -= weight;
    }

    listOfcomponentRegistered[indexComponent].capacity = resNormalizeWeight(listOfcomponentRegistered[indexComponent].capacity);
}


float getCoreConsumedCapacity(int indexComponent)
{
    int i = 0;
    float consumedCapacity = 0.0f;
    int refCoreIndex = resGetCorrespondCoreIndex(listOfcomponentRegistered[indexComponent].component_name);

    if (listOfcomponentRegistered) {
        while(listOfcomponentRegistered[i].component_name != NULL ) {

            DEBUG(DEB_LEV_SIMPLE_SEQ, "In %s, current indexComponent %d, name is %s, remain capacity is %f\n",
                    __func__, indexComponent, listOfcomponentRegistered[i].component_name, listOfcomponentRegistered[i].capacity);
            if (refCoreIndex == resGetCorrespondCoreIndex(listOfcomponentRegistered[i].component_name)) {
                consumedCapacity += (MAX_COMPONENTS_CAPACITY - listOfcomponentRegistered[i].capacity);
            }
            i++;
        }
    }
...
OMX_BOOL checkComponentResource(OMX_COMPONENTTYPE *openmaxStandComp, int indexComponent)
{
    int width, height, framerate;
    float weight = 0.0f, consumedCapacity = 0.0f;
    ...
    OMX_PARAM_PORTDEFINITIONTYPE *pPortParam = &omx_base_component_Private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam;

    width = pPortParam->format.video.nFrameWidth;
    height = pPortParam->format.video.nFrameHeight;
    framerate = pPortParam->format.video.xFramerate >> 16;
    //first, we will check the remained capacity
    weight = resGetCorrespondWeight(omx_base_component_Private->name, width, height, framerate);

    //get the consumed capacities of correspond core
    consumedCapacity = getCoreConsumedCapacity(indexComponent);
    DEBUG(DEB_LEV_SIMPLE_SEQ, "In %s, current comp weight is %f, needed is %f, core consumed capacity is %f\n",
            __func__, listOfcomponentRegistered[indexComponent].capacity, weight, consumedCapacity);

    if (consumedCapacity + weight > MAX_COMPONENTS_CAPACITY) {
        DEBUG(DEB_LEV_ERR, "In %s, not enough capacity, current used is %f, needed is %f, return with error\n", __func__, consumedCapacity, weight);
        return OMX_FALSE;
    }

    //if there is still capacity, check the num
    if (numElemInList(globalComponentList[indexComponent]) >= listOfcomponentRegistered[indexComponent].max_components) {
        DEBUG(DEB_LEV_ERR, "In %s, exeed the num limit, return with error\n", __func__);
        return OMX_FALSE;
    }

    omx_base_component_Private->fWeight = weight;
    updateResourceCapacity(weight, indexComponent, OMX_FALSE);
    return OMX_TRUE;
}
...
OMX_ERRORTYPE RM_getResource(OMX_COMPONENTTYPE *openmaxStandComp) {
    ComponentListType *componentCandidate = NULL;
    omx_base_component_PrivateType* omx_base_component_Private;
    int candidates;
    OMX_ERRORTYPE err;
    int i = 0;
    ...
    if (!checkComponentResource(openmaxStandComp, indexComponent)) {
        ...
        if (candidates != OMX_ErrorUndefined && componentCandidate != NULL) {
        ...
        err = addElemToList(&globalComponentList[indexComponent], openmaxStandComp, indexComponent, OMX_FALSE);
        if(err == OMX_ErrorNone) {
            omx_base_component_Private->bIsResourced = OMX_TRUE;

#ifdef ANDROID
            DEBUG(DEB_LEV_SIMPLE_SEQ, "%s acquirePerfLock\n", __func__);
            acquirePerfLock(0, 0);
#endif
        }
    }
...
OMX_ERRORTYPE RM_releaseResource(OMX_COMPONENTTYPE *openmaxStandComp){
    ...
    OMX_ERRORTYPE err;
    int i = 0, indexComponent = -1;
    OMX_PARAM_PORTDEFINITIONTYPE *pPortParam = NULL;

    DEBUG(DEB_LEV_FUNCTION_NAME, "In %s\n", __func__);
    omx_base_component_Private = (omx_base_component_PrivateType*)openmaxStandComp->pComponentPrivate;
    pPortParam = &omx_base_component_Private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam;

    if (!omx_base_component_Private->bIsResourced) {
        // No resource to be handled
        DEBUG(DEB_LEV_ERR, "In %s , resouce not getted, No resource to be handled\n", __func__);
        return OMX_ErrorNone;
    }
    ...
    omx_base_component_Private->bIsResourced = OMX_FALSE;

    if (!hasActivedComponent())
    {
#ifdef ANDROID
        DEBUG(DEB_LEV_SIMPLE_SEQ, "In %s, no comp in use, so releasePerfLock\n", __func__);
        releasePerfLock(CpuBoostCancelRequest);
#endif
    }

    updateResourceCapacity(omx_base_component_Private->fWeight, indexComponent, OMX_TRUE);
    DEBUG(DEB_LEV_SIMPLE_SEQ, "In %s, capacity restore to %f \n", __func__, listOfcomponentRegistered[indexComponent].capacity);
