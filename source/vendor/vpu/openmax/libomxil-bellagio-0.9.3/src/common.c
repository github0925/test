...
#ifdef ANDROID
#ifndef REGISTRY_FILE_PATH
#define REGISTRY_FILE_PATH "/vendor/etc/"
#endif
#else
#ifndef REGISTRY_FILE_PATH
#define REGISTRY_FILE_PATH ""
#endif
#endif
#else
#ifdef ANDROID
#define TMP_DATA_DIRECTORY "/vendor/etc/"
#else
...
#ifdef REGISTRY_FILE_PATH
	omxregistryfile  = malloc(strlen(REGISTRY_FILE_PATH) + strlen(REGISTRY_FILENAME) + 2);
	if (!omxregistryfile) {
	  printf("Insufficient memory in %s\n", __func__);
	  return NULL;
  	}
	strcpy(omxregistryfile, REGISTRY_FILE_PATH);
#else
...
#ifdef REGISTRY_FILE_PATH
	omxregistryfile  = malloc(strlen(REGISTRY_FILE_PATH) + strlen(registry_name) + 2);
	if (!omxregistryfile) {
		printf("Insufficient memory in %s\n", __func__);
		return NULL;
	}
