...
#define omx_base_component_PrivateType_FIELDS \
	...
	OMX_BOOL bIsResourced;  /** @param bIsResourced boolean flag is true when resouce is  getted*/ \
	volatile OMX_BOOL bIsBufMgThreadExit;  /** @param bIsBufMgThreadExit boolean flag is true when the buf manager thread shoud be exited*/ \
	float    fWeight;       /** @param fWeight the weight of current componet*/ \
