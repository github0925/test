...
void* omx_base_source_BufferMgmtFunction (void* param) {
...
#if defined(WIN32)
  omx_base_source_Private->bellagioThreads->nThreadBufferMngtID = (long int)(long int)GetCurrentThreadId();
#else
  ...
  while(omx_base_component_Private->state == OMX_StateIdle || omx_base_component_Private->state == OMX_StateExecuting ||
    ...
    if((isOutputBufferNeeded==OMX_TRUE && pOutputSem->semval==0) &&
		...
		(omx_base_source_Private->state != OMX_StateLoaded)) {
#else
...
#ifdef SUPPORT_CM_OMX_12
	if(omx_base_source_Private->state == OMX_StateLoaded) {
#else
