...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
	...
	omx_base_component_Private->bIsBufMgThreadExit = OMX_FALSE;
	...
	if (err) {
		DEBUG(DEB_LEV_ERR, "Starting message handler thread failed\n");
		base_constructor_remove_garbage_collected(omx_base_component_Private);
		return OMX_ErrorInsufficientResources;
	}
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_Destructor(OMX_COMPONENTTYPE *openmaxStandComp) {
  ...
  int ret = pthread_kill(omx_base_component_Private->messageHandlerThread, 0);
  if (ret != ESRCH) {
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_ComponentDeInit(
  ...
  if (!omx_base_component_Private->bIsBufMgThreadExit && omx_base_component_Private->bufferMgmtThreadID == 0) {
    DEBUG(DEB_LEV_ERR,"In %s for component %p, a force release may be happened \n", __func__, openmaxStandComp);

    /*Signal Buffer Management thread to exit*/
    omx_base_component_Private->bIsBufMgThreadExit = OMX_TRUE;

    /*up twice, cause the buf manager thread may block on the input buffer*/
    tsem_up(omx_base_component_Private->bMgmtSem);
    tsem_up(omx_base_component_Private->bMgmtSem);
    DEBUG(DEB_LEV_ERR,"In %s waiting for component buffer manager thread %d to exit\n",
          __func__, (int)omx_base_component_Private->bellagioThreads->nThreadBufferMngtID);

    int ret = pthread_kill(omx_base_component_Private->bufferMgmtThread, 0);
    // the correspond thread is exist
    if (ret != ESRCH) {
        pthread_join(omx_base_component_Private->bufferMgmtThread, NULL);
    }
    omx_base_component_Private->bufferMgmtThreadID = -1;

    DEBUG(DEB_LEV_ERR,"In %s end for component buffer manager thread %d to exit\n",
          __func__, (int)omx_base_component_Private->bellagioThreads->nThreadBufferMngtID);
  }

  // if the componet have been resourced, the resource should be released
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_DoStateSet(OMX_COMPONENTTYPE *openmaxStandComp, OMX_U32 destinationState) {
  ...
  if(destinationState == OMX_StateIdle){
    ...
    case OMX_StateLoaded:
             ...
             if(PORT_IS_ENABLED(pPort))
             {
               ...
               if (pPort->sPortParam.nBufferCountActual > 0)
               {
                ...
                if (pPort->bIsDestroying
                     || pPort->nNumAssignedBuffers != pPort->sPortParam.nBufferCountActual)
                {
                   bExit = OMX_TRUE;
                   pthread_mutex_unlock(&pPort->exitMutex);
                   DEBUG(DEB_LEV_ERR, "In %s Allocating Buffer Error with bIsDestroying %d, buf[%d/%d]\n", __func__,
                         pPort->bIsDestroying, pPort->nNumAssignedBuffers, pPort->sPortParam.nBufferCountActual);
                   continue;
                 }
                pthread_mutex_unlock(&pPort->exitMutex);
              }
      ...
      if (bExit) {
        DEBUG(DEB_LEV_ERR, "bExit = true!! break!!\n");
        break;
      }
...
OSCL_EXPORT_REF OMX_ERRORTYPE checkHeader(OMX_PTR header, OMX_U32 size) {
  ...
  if(ver->s.nVersionMajor != SPECVERSIONMAJOR ||
    ...
    DEBUG(DEB_LEV_ERR, "The version does not match, ver %d.%d, define %d.%d\n",
        ver->s.nVersionMajor, ver->s.nVersionMinor, SPECVERSIONMAJOR, SPECVERSIONMINOR);
...
#ifdef SUPPORT_CM_OMX_12
	else if ((nParam == OMX_StateExecuting) && (omx_base_component_Private->state == OMX_StateIdle)) {
		omx_base_component_Private->transientState = OMX_TransStateIdleToExecuting;
	}
#endif
    break;
...
void* compMessageHandlerFunction(void* param) {
  ...
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s for component %p\n", __func__, openmaxStandComp);
#if defined(WIN32)
  omx_base_component_Private->bellagioThreads->nThreadMessageID = 0 ;//(long int) omx_base_component_Private->messageHandlerThread;
#else
  ...
  while(1){
	...
	DEBUG(DEB_LEV_FULL_SEQ, "omx_base_component_Private->messageSem->num : %d\n",omx_base_component_Private->messageSem->semval);
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_MessageHandler(OMX_COMPONENTTYPE *openmaxStandComp,internalRequestMessageType* message) {
  ...
  */
  ...
  case OMX_CommandStateSet: {
    ...
    retState = (err == OMX_ErrorNone) ? message->messageParam : omx_base_component_Private->state;
    DEBUG(DEB_LEV_SIMPLE_SEQ, "In %s, after DoStateSet, componet state is %d\n", __func__, retState);
    ...
    retState, /* The state of openmaxStandComp */
    NULL);
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_EmptyThisBuffer(
  ...
  if (err != OMX_ErrorNone) {
#if defined(WIN32)
	  DEBUG(DEB_LEV_ERR, "Out of %s for component %p\n", __func__, hComponent);
#else
