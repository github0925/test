...
static int showComponentsList(FILE* omxregistryfp) {
	...
	memset(buffer, 0x00, allocation_length+1);
	comp_name = malloc(DEFAULT_LINE_LENGHT);
	memset(comp_name, 0x00, DEFAULT_LINE_LENGHT);
	temp_name = malloc(DEFAULT_LINE_LENGHT);
	memset(temp_name, 0x00, DEFAULT_LINE_LENGHT);
	comp_rules = malloc(DEFAULT_LINE_LENGHT);
	memset(comp_rules, 0x00, DEFAULT_LINE_LENGHT);
	checkChar = malloc(2);
	memset(checkChar, 0x00, 2);
	...
	while(1) {
		...
		buffer[offset] = '\0';
#if defined(WIN32)
		if (buffer[0] == '/' || buffer[0] == 'C' || buffer[0] == 'D' || buffer[0] == ':') {	// strip d:/project
			continue;
		}
#else
		if (buffer[0] == '/') {
...
static int buildComponentsList(FILE* omxregistryfp, char *componentspath, int verbose) {
	...
	memset(qualityString, 0x00, 4096);
	buffer = malloc(8192);
...
#else
				if(strncmp(dp->d_name, "libomx", 6) == 0)
#endif
				{
#if defined(WIN32)
					char lib_absolute_path[2048];
#else
...
#if defined(WIN32)
					if((handle = LoadLibrary(lib_absolute_path)) == NULL)
#else
					if((handle = dlopen(lib_absolute_path, RTLD_NOW)) == NULL)
#endif
					{
#if defined(WIN32)
						DEBUG(DEB_LEV_ERR, "could not load %s, erro=%s\n", lib_absolute_path);
#else
...
#endif
						{
							DEBUG(DEB_LEV_SIMPLE_SEQ, "the library %s is not compatible with ST static component loader\n", lib_absolute_path);
							continue;
						}
						num_of_libraries++;
