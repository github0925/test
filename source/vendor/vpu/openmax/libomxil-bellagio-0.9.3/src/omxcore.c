...
//OMX_ContentPipe.h has been removed
#else
...
OSCL_EXPORT_REF OMX_ERRORTYPE OMX_GetHandle(OMX_HANDLETYPE* pHandle,
  ...
  int i;
  GetDebugLevelFromProperty(DEBUG_LOG_CORE, DEB_LEV_ERR);
...
/** @brief the OMX_RoleOfComponentEnum standard function
 */
OSCL_EXPORT_REF OMX_ERRORTYPE OMX_RoleOfComponentEnum (
  OMX_STRING role,
  OMX_STRING compName,
  OMX_U32 nIndex) {
	...
	for (i = 0; i < bosa_loaders; i++) {
		err = loadersList[i]->BOSA_GetRoleOfComponentEnum(
			loadersList[i],
			role,
			compName,
			nIndex);
...
OSCL_EXPORT_REF OMX_ERRORTYPE OMX_ComponentOfRoleEnum  (
	OMX_STRING compName,
	OMX_STRING role,
	OMX_U32 nIndex)
{
	...
	for (i = 0; i < bosa_loaders; i++) {
		err = loadersList[i]->BOSA_GetComponentOfRoleEnum(
			loadersList[i],
			compName,
			role,
			nIndex);
