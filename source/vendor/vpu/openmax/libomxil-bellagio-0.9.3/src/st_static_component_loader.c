...
void st_static_setup_component_loader(BOSA_COMPONENTLOADER * st_static_loader) {
	  ...
	  st_static_loader->BOSA_GetRoleOfComponentEnum = &BOSA_ST_GetRoleOfComponentEnum;
	  st_static_loader->BOSA_GetComponentOfRoleEnum = &BOSA_ST_GetComponentOfRoleEnum;
#else
...
OMX_ERRORTYPE BOSA_ST_InitComponentLoader(BOSA_COMPONENTLOADER *loader) {
  ...
  if (omxregistryfp == NULL){
    DEBUG(DEB_LEV_ERR, "st loader aaaaa Cannot open OpenMAX registry file %s\n", registry_filename);
    return ENOENT;
  }
  ...
  while(1) {
	  ...
	  *(line+index_readline) = '\0';
...
#if defined(WIN32)
	  if((handle = (void *)LoadLibrary(libname)) == NULL)
#else
	  if((handle = dlopen(libname, RTLD_NOW)) == NULL)
#endif
	  {
#if defined(WIN32)
		  DEBUG(DEB_LEV_ERR, "could not load %s\n", libname);
#else
	  ...
	  else
	  {
		  ...
		  numLib++;
#if defined(WIN32)
		  if ((fptr = (void *)GetProcAddress(handle, "omx_component_library_Setup")) == NULL)
#else
		  if ((fptr = dlsym(handle, "omx_component_library_Setup")) == NULL)
#endif
		  {
#if defined(WIN32)
			  DEBUG(DEB_LEV_ERR, "the library %s is not compatible with ST static component loader\n", libname);
#else
...
OMX_ERRORTYPE BOSA_ST_DeInitComponentLoader(BOSA_COMPONENTLOADER *loader) {
  ...
  for(i=0;i<numLib;i++) {
#if defined(WIN32)
	  err = FreeLibrary(handleLibList[i]);
#else
...
#if defined(WIN32)
	  templateList[componentPosition]->name_requested = strdup (cComponentName);
#else
...
#ifdef SUPPORT_CM_OMX_12
OMX_ERRORTYPE BOSA_ST_GetRoleOfComponentEnum(
	BOSA_COMPONENTLOADER *loader,
	OMX_STRING role,
	OMX_STRING compName,
	OMX_U32 nIndex)
{
	...
	while (templateList[i]) {
		...
		if(!strcmp(templateList[i]->name, compName)) {
			...
			//find the role by index
			for (index = 0; index < templateList[i]->name_specific_length; index++) {
				if (nIndex == index) {
					strcpy(role, templateList[i]->role_specific[index]);
					DEBUG(DEB_LEV_SIMPLE_SEQ, "Found requested roles by template %s\n", templateList[i]->role_specific[index]);
					found = 1;
					break;
				}
			}
			...
			for(j=0;j<templateList[i]->name_specific_length;j++) {
				if(!strcmp(templateList[i]-> name_specific[j], compName)) {
					...
					if (nIndex == j) {
						strcpy (role , templateList[i]->role_specific[j]);
						DEBUG(DEB_LEV_SIMPLE_SEQ, "Found requested roles by role_specific %s\n", templateList[i]->role_specific[j]);
						found = 1;
						break;
					}
				}
	...
	if(!found) {
		DEBUG(DEB_LEV_ERR, "no component match in whole template list has been found nIndex=%d\n", (int)nIndex);
		return OMX_ErrorNoMore;
	}
	DEBUG(DEB_LEV_FUNCTION_NAME, "Out of %s\n", __func__);
...
/** @brief The specific version of OMX_ComponentOfRoleEnum
 *
 * This function replicates exactly the behavior of the
 * standard OMX_ComponentOfRoleEnum function for the ST static
 * component loader
 */


OMX_ERRORTYPE BOSA_ST_GetComponentOfRoleEnum (
  BOSA_COMPONENTLOADER *loader,
  OMX_STRING compName,
  OMX_STRING role,
  OMX_U32 nIndex) {
  ...
  while(templateList[i]) {
    for (j = 0; j<templateList[i]->name_specific_length; j++) {
      if (!strcmp(templateList[i]->role_specific[j], role)) {
		  if (nIndex == (OMX_U32)num_comp) {
			strcpy(compName, templateList[i]->name);
			found = 1;
		  }
        num_comp++;
      }
  ...
  if (!found) {
	  DEBUG(DEB_LEV_ERR, "no Role match in whole template list has been found nIndex=%d\n", (int)nIndex);
	  return OMX_ErrorNoMore;
  }
