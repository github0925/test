...
#ifdef WITH_JPU_HW
    //for jpu, the input buffer is set to YUV444
    if (cinfo->hw_encompress)
    {
      int ret = jpu_encompress_fill_input_buffer(cinfo->jpu, main_ptr->buffer[0],
        main_ptr->buffer[1], main_ptr->buffer[2], DCTSIZE);
