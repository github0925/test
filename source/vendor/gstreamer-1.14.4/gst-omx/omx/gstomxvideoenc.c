...
#ifdef USE_OMX_TARGET_SDRV
static gboolean gst_omx_video_enc_set_use_buffer (GstOMXVideoEnc * self);
gboolean
gst_omx_buffer_import_fd_sdrv_ext (GstOMXBuffer * buffer, GstBuffer * input);
#endif
...
#ifdef USE_OMX_TARGET_SDRV
  if (self->fd_list) {
    g_list_foreach (self->fd_list, (GFunc) g_free, NULL);
  }
  g_list_free (self->fd_list);
  self->fd_list = NULL;
#endif
    ...
    GstOMXBuffer * buf, GstVideoCodecFrame * frame)
{
  ...
  if ((buf->omx_buf->nFlags & OMX_BUFFERFLAG_CODECCONFIG)
    ...
    if (self->headers) {
      ...
      GST_DEBUG_OBJECT (self, "Write the buffer headers to data frame");
...
#elif defined(USE_OMX_TARGET_SDRV)
    if (gst_is_dmabuf_memory (gst_buffer_peek_memory (input, 0))) {
      self->input_dmabuf = TRUE;

      GST_DEBUG_OBJECT (self, "use dmabuf mode %d", self->input_allocation);
    }
#endif
  ...
  if (self->disabled) {
    ...
    if (!(klass->cdata.hacks & GST_OMX_HACK_NO_DISABLE_OUTPORT)) {
      ...
      if (self->input_dmabuf) {
        ...
        /* Fall to normal way, allocate buffers from omx */
        if (!gst_omx_video_enc_allocate_in_buffers (self))
          return FALSE;
      }
    #else
    ...
    GstOMXBuffer * outbuf)
{
  ...
  if (self->input_dmabuf) {
    /* dmabuf input */
    if (!gst_omx_buffer_import_fd_sdrv_ext (outbuf, inbuf)) {
      GST_ELEMENT_ERROR (self, STREAM, FORMAT, (NULL),
          ("failed to import dmabuf"));
      return FALSE;
    }
    ...
    GST_LOG_OBJECT (self, "Import dmabuf with %p, fd is %d",
        outbuf->omx_buf->pBuffer, *(guint *) outbuf->omx_buf->pBuffer);
...
gst_omx_video_enc_getcaps (GstVideoEncoder * encoder, GstCaps * filter)
{
  ...
  n = gst_caps_get_size(comp_supported_caps);
  for (guint i = 0; i < n; i++ ) {
      gst_caps_set_features (comp_supported_caps, i, gst_caps_features_new ("memory:DMABuf", NULL));
  }
...
gst_omx_video_enc_set_use_buffer (GstOMXVideoEnc * self)
{
  ...
  /* step 1: dma mode setting */
  {
    /* here we hack the buf mode of omx, send a portformattype to omx through
       a new index, in order to let the omx know, the deliviered buf is with
       mode dma
     */
    OMX_VIDEO_PARAM_PORTFORMATTYPE param;
    GST_OMX_INIT_STRUCT (&param);
    param.nPortIndex = self->enc_in_port->index;
    param.eCompressionFormat = OMX_VIDEO_CodingVendorDmaMode;

    err =
        gst_omx_component_set_parameter (self->enc_in_port->comp,
        (OMX_INDEXTYPE) OMX_Extension_IndexPortParamBufferMode, &param);

    if (err != OMX_ErrorNone) {
      GST_WARNING_OBJECT (self->enc_in_port->comp->parent,
          "Failed to set port %d to dmabuf mode: %s (0x%08x)",
          self->enc_in_port->index, gst_omx_error_to_string (err), err);
      return FALSE;
    }
  }

  /* step 2: populated the buf */
  gst_omx_port_get_port_definition (self->enc_in_port, &port_def);

  /* allocate the buf list with fd size, not the acutal buf fd */
  for (i = 0; i < port_def.nBufferCountActual; i++) {
    guint *pfd = g_new (guint, 1);
    *pfd = 0;                   /* useless value, just for error check */

    addr = g_list_append ((GList *) addr, pfd);
  }
...
gst_omx_buffer_import_fd_sdrv_ext (GstOMXBuffer * buffer, GstBuffer * input)
{
  ...
  *(guint *) buffer->omx_buf->pBuffer = fd;
