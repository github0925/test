    ...
    GstBuffer ** buffer, GstBufferPoolAcquireParams * params)
{
  ...
  if (pool->other_pool) {
    ...
    if (pool->output_mode == GST_OMX_BUFFER_MODE_DMABUF) {
      ...
      fd = *((guint *) (omx_buf->omx_buf->pBuffer));
      GST_DEBUG_OBJECT (pool, "dmabuf mapping to fd %d", fd);
...
gst_omx_buffer_pool_free_buffer (GstBufferPool * bpool, GstBuffer * buffer)
{
  ...
  if (gst_is_dmabuf_memory (mem)) {
    gint prime_fd = gst_dmabuf_memory_get_fd (mem);
    gst_omx_buffer_bo_free (bpool, prime_fd);
  }
...
gst_omx_buffer_pool_finalize (GObject * object)
{
  ...
  if (pool->prime_fds) {
    g_list_free_full (pool->prime_fds, g_free);
  }
    ...
    GstOMXPort * port, GstOMXBufferMode output_mode)
{
  ...
  if (pool->output_mode == GST_OMX_BUFFER_MODE_DMABUF) {
    pool->fd = drmOpen ("semidrive", NULL);
    if (pool->fd < 0) {
      GST_ELEMENT_ERROR (pool, RESOURCE, OPEN_READ_WRITE,
          ("Could not open DRM module %s", GST_STR_NULL ("semidrive")),
          ("reason: %s (%d)", strerror (errno), errno));
      g_assert_not_reached ();
    } else {
      GST_DEBUG_OBJECT (pool, "pool->fd %d", pool->fd);
    }
  }
...
gboolean
gst_omx_buffer_bo_alloc (GstBufferPool * object, GstVideoInfo * vinfo,
    guint * prime_fd)
{
  GstOMXBufferPool *pool = GST_OMX_BUFFER_POOL (object);
  if (pool->fd < 0) {
  ...
  guint32 fmt = gst_drm_format_from_video (GST_VIDEO_INFO_FORMAT (vinfo));
  arg.bpp = gst_drm_bpp_from_drm (fmt);
  arg.width = GST_VIDEO_INFO_WIDTH (vinfo);
  h = GST_VIDEO_INFO_HEIGHT (vinfo);
  arg.height = gst_drm_height_from_drm (fmt, h);

  ret = drmIoctl (pool->fd, DRM_IOCTL_MODE_CREATE_DUMB, &arg);
  if (ret) {
    goto create_failed;
  }

  ret = drmPrimeHandleToFD (pool->fd, arg.handle,
      DRM_CLOEXEC | DRM_RDWR, prime_fd);
  if (ret) {
    drmIoctl (pool->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &arg);
    goto create_failed;
  }

  GST_DEBUG_OBJECT (pool, "dmabuf prime_fd %d", *prime_fd);
...
create_failed:
  {
    GST_ERROR_OBJECT (pool, "Failed to create buffer object: %s (%d)",
        strerror (-ret), ret);
    return FALSE;
  }
}
...
gboolean
gst_omx_buffer_bo_free (GstBufferPool * object, guint prime_fd)
{
  GstOMXBufferPool *pool = GST_OMX_BUFFER_POOL (object);
  if (pool->fd < 0) {
    return FALSE;
  ...
  gint ret = drmPrimeFDToHandle (pool->fd, prime_fd, &arg.handle);
  if (ret) {
    GST_ERROR_OBJECT (pool, "Failed to import prime fd %d: %s (%d)",
        prime_fd, strerror (-ret), ret);
    return FALSE;
  } else {
    ret = drmIoctl (pool->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &arg);
    if (ret) {
      GST_ERROR_OBJECT (pool, "Failed to destroy buffer object: %s (%d)",
          strerror (-ret), ret);
      return FALSE;
    }
  }
