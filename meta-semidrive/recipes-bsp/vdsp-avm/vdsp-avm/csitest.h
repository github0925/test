...
public:
    ...
    int init_buffer();
    int init_buffer(uint8_t** drm_ptr, size_t buf_len);
    int set_stream(int on);
    int get_frame(uint8_t **buf);
    int get_frame(uint8_t **buf, int *idx);
    int get_userptr_frame(uint8_t **buf, int *idx);
...
public:
    ...
    ~NetVdsp(){};
    int Init(std::vector<size_t> &szIn);
    int Init(std::vector<size_t> &szIn, std::vector<size_t> &szOut);
    int Init(std::vector<void *> &pIn,  std::vector<size_t> &szIn, std::vector<size_t> &szOut);
    int Init(std::vector<void *> &pIn,  std::vector<size_t> &szIn, std::vector<void *> &pOut, std::vector<size_t> &szOut);
    void DeInit();
    int startInf(XtensaOperation* cmd, std::vector<void *> &pIn, std::vector<size_t> &szIn);
    int startInf(XtensaOperation* cmd, std::vector<void *> &pIn, std::vector<size_t> &szIn, std::vector<void *> &pOut, std::vector<size_t> &szOut);
    int startInf(XtensaOperation* cmd);
    int mapOutput(std::vector<void*> &pMap);
    int unmapOutput(std::vector<void*> &pmap);
    int mapInput(std::vector<void*> &pMap);
    int unmapInput(std::vector<void*> &pmap);
private:
    xrp_buffer_group *bufferGroup;
    std::vector<xrp_buffer*> inputBuf;
    std::vector<size_t> inputSz;
    std::vector<size_t> inputGrpIdx;
