...
int _g2d_convert(G2DInput *pInput)
{
	...
	for (int i = 0; i < pInput->layer_num; i++) {
		...
		layer->dst_x = pInput->layer[i].dst_crop_x;
		layer->dst_y = pInput->layer[i].dst_crop_y;
		layer->dst_w = pInput->layer[i].dst_crop_w;
		layer->dst_h = pInput->layer[i].dst_crop_h;
        slog_info("[dump] %s *ENABLE = %d*,  format: 0x%x	 source (%d, %d, %d, %d) => dest (%d, %d, %d, %d)\n",
			__func__, layer->enable, layer->format,
...
int g2d_format_convert(int      src_format,int dst_format,int src_fd,int dst_fd,int width,int height)
{
    ...
    g2dinput.layer[0].dst_crop_x = 0;
    g2dinput.layer[0].dst_crop_y =0;
    g2dinput.layer[0].dst_crop_w = width;
    g2dinput.layer[0].dst_crop_h = height;
    g2dinput.rotation = 0;
...
int g2d_format_scale(int src_format,int dst_format,int src_fd,int dst_fd,int src_w,int src_h,int dst_w,int dst_h)
{
    ...
    g2dinput.layer[0].dst_crop_x = 0;
    g2dinput.layer[0].dst_crop_y =0;
    g2dinput.layer[0].dst_crop_w = dst_w;
    g2dinput.layer[0].dst_crop_h = dst_h;
    g2dinput.rotation = 0;
    g2dinput.layer[0].src_format = src_format;
    g2dinput.dst_format = dst_format;
