...
#undef PAGE_ALIGN
	...
	if (ret) {
		LOGE("export (0x%lx) dmabuf failed: %d(%s)", (long)addr, ret, strerror(ret));
	}
...
int DrmFrame::MapBO()
{
	...
	if (ret)
		return ret;
	size_t sz_bo = (bo.pitches[0] + bo.pitches[1]) * bo.height;
...
void DrmPlane::getProperties() {
	...
	property_pd_type = get_property_id(drm_fd, props, "pd_type");
	property_pd_mode = get_property_id(drm_fd, props, "pd_mode");
...
int DrmPlane::setPlane(uint32_t crtc_id, drmModeAtomicReq *req, DrmFrame *frame) {
	...
	drmModeAtomicAddProperty(req, plane_id, property_pd_type, frame->pd_type);
	drmModeAtomicAddProperty(req, plane_id, property_pd_mode, frame->pd_mode);
...
int DrmBackend::PostAsync(int crtc_id, DrmPlane *plane, DrmFrame *frame, int *fencefd) {
	return PostAsync(crtc_id, 1, &plane, &frame, fencefd);
}

int DrmBackend::PostAsync(int crtc_id, int num_frame, DrmPlane **planes, DrmFrame **frames, int *fencefd) {
	...
	for (int i = 0; i < num_frame; i++) {
		DrmPlane *plane = planes[i];
		DrmFrame *frame = frames[i];
...
int test_nv12(int argc, char const *argv[])
{
	...
	int ret = frame->createDumbBuffer(400, 300, DRM_FORMAT_ABGR8888, 0, 0);
	if (ret) {
	...
	while (n--) {
		...
		drm_ioctl_export_dmabuf(backend.fd(), phys_address, 400 * 300 * 4, &prime_fd, 0);
		DDBG(prime_fd);
		new_frame->ImportBuffer(400, 300, 400, DRM_FORMAT_ABGR8888, prime_fd, 0, 0);
		new_frame->addFrameBuffer(0);
		printf("frame->mapped_vaddr = %p\n", frame->mapped_vaddr);
		//if (n > 998)
			setColor(frame->mapped_vaddr, 400, 300, 0xffffffff);
