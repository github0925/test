...
static int open_device(const char *device, int print_flags)
{
    ...
    new_ufds = static_cast<struct pollfd *>(realloc(ufds, sizeof(ufds[0]) * (nfds + 1)));
    if(new_ufds == NULL) {
        fprintf(stderr, "out of memory\n");
        return -1;
    }
    ufds = new_ufds;
    new_device_names = static_cast<char **>(realloc(device_names, sizeof(device_names[0]) * (nfds + 1)));
...
int main(int argc, char *argv[])
{
    ...
    nfds = 1;
    ufds = static_cast<struct pollfd *>(calloc(1, sizeof(ufds[0])));
