...
inherit externalsrc
EXTERNALSRC = "${BSPDIR}/source/openssl"
...
PACKAGECONFIG[cryptodev-linux] = "disable-devcryptoeng,enable-devcryptoeng,cryptodev-linux,,cryptodev-module"
...
# srp (libest)
# whirlpool (qca)
DEPRECATED_CRYPTO_FLAGS = "no-ssl no-idea no-rc5 no-md2 no-camellia no-mdc2 no-scrypt no-seed no-siphash"
...
do_configure () {
	...
	*)
	...
	perl ${S}/Configure ${EXTRA_OECONF} ${PACKAGECONFIG_CONFARGS} ${DEPRECATED_CRYPTO_FLAGS} --prefix=$useprefix --openssldir=${libdir}/ssl-1.1 --libdir=${libdir} $target enable-devcryptoeng enable-engine enable-dso enable-afalgeng
